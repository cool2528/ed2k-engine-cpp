...
class ByteReader {
 ...
 public:
  ...
  [[nodiscard]] std::uint8_t  u8() noexcept { if(!need(1))return 0; return std::to_integer<std::uint8_t>(buf_[pos_++]); }
  [[nodiscard]] std::uint16_t u16() noexcept { if(!need(2))return 0; std::uint16_t v=std::to_integer<std::uint8_t>(buf_[pos_])
    |(std::uint16_t(std::to_integer<std::uint8_t>(buf_[pos_+1]))<<8); pos_+=2; return v; }
  [[nodiscard]] std::uint32_t u32() noexcept { if(!need(4))return 0; std::uint32_t v=0; for(int i=0;i<4;++i)
    v|=std::uint32_t(std::to_integer<std::uint8_t>(buf_[pos_+i]))<<(8*i); pos_+=4; return v; }
  [[nodiscard]] std::uint32_t u32_be() noexcept { if(!need(4))return 0; std::uint32_t v=0; for(int i=0;i<4;++i)
    v=(v<<8)|std::to_integer<std::uint8_t>(buf_[pos_+i]); pos_+=4; return v; }   // Network byte order IP (a.b.c.d, a in high byte)
  [[nodiscard]] std::uint64_t u64() noexcept { if(!need(8))return 0; std::uint64_t v=0; for(int i=0;i<8;++i)
    v|=std::uint64_t(std::to_integer<std::uint8_t>(buf_[pos_+i]))<<(8*i); pos_+=8; return v; }
  [[nodiscard]] std::string string16(){ std::uint16_t n=u16(); if(!need(n))return {};
    std::string s(reinterpret_cast<const char*>(buf_.data()+pos_),n); pos_+=n; return s; }
  [[nodiscard]] MD4Hash hash16() noexcept { if(!need(16))return {}; std::array<std::byte,16> h;
    for(int i=0;i<16;++i)h[i]=buf_[pos_+i]; pos_+=16; return MD4Hash::from_bytes(h); }
  [[nodiscard]] std::array<std::byte,20> hash20() noexcept { std::array<std::byte,20> h{}; if(!need(20))return h;
    for(int i=0;i<20;++i)h[i]=buf_[pos_+i]; pos_+=20; return h; }
  [[nodiscard]] std::span<const std::byte> blob(std::size_t n) noexcept { if(!need(n))return {};
...
class ByteWriter {
 ...
 public:
  ...
  void u32_be(std::uint32_t v){ for(int i=3;i>=0;--i) out_.push_back(std::byte((v>>(8*i))&0xff)); } // Network byte order IP (a.b.c.d)
