...
TEST(Smoke, VersionMatchesRelease) { EXPECT_EQ(ed2k::version(), "2.7.4"); }
TEST(Smoke, CompileTimeVersionConstants) {
  static_assert(ed2k::version_major == 2);
  static_assert(ed2k::version_minor == 7);
  static_assert(ed2k::version_patch == 4);
  static_assert(ED2K_VERSION_AT_LEAST(2, 7, 4));
  static_assert(ED2K_VERSION_AT_LEAST(2, 1, 0));
  static_assert(!ED2K_VERSION_AT_LEAST(2, 7, 5));
