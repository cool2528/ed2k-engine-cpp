...
TEST(Smoke, VersionMatchesRelease) { EXPECT_EQ(ed2k::version(), "2.3.0"); }
TEST(Smoke, CompileTimeVersionConstants) {
  static_assert(ed2k::version_major == 2);
  static_assert(ed2k::version_minor == 3);
  static_assert(ed2k::version_patch == 0);
  static_assert(ED2K_VERSION_AT_LEAST(2, 3, 0));
  static_assert(ED2K_VERSION_AT_LEAST(2, 1, 0));
  static_assert(!ED2K_VERSION_AT_LEAST(2, 4, 0));
