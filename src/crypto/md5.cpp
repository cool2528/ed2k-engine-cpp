#include "ed2k/crypto/md5.hpp"

#include <array>
#include <cstdint>
#include <vector>

namespace ed2k::crypto {
namespace {
constexpr std::array<std::uint32_t, 64> k_shift = {
    7, 12, 17, 22, 7, 12, 17, 22, 7, 12, 17, 22, 7, 12, 17, 22,
    5, 9,  14, 20, 5, 9,  14, 20, 5, 9,  14, 20, 5, 9,  14, 20,
    4, 11, 16, 23, 4, 11, 16, 23, 4, 11, 16, 23, 4, 11, 16, 23,
    6, 10, 15, 21, 6, 10, 15, 21, 6, 10, 15, 21, 6, 10, 15, 21,
};

constexpr std::array<std::uint32_t, 64> k_table = {
    0xd76aa478, 0xe8c7b756, 0x242070db, 0xc1bdceee,
    0xf57c0faf, 0x4787c62a, 0xa8304613, 0xfd469501,
    0x698098d8, 0x8b44f7af, 0xffff5bb1, 0x895cd7be,
    0x6b901122, 0xfd987193, 0xa679438e, 0x49b40821,
    0xf61e2562, 0xc040b340, 0x265e5a51, 0xe9b6c7aa,
    0xd62f105d, 0x02441453, 0xd8a1e681, 0xe7d3fbc8,
    0x21e1cde6, 0xc33707d6, 0xf4d50d87, 0x455a14ed,
    0xa9e3e905, 0xfcefa3f8, 0x676f02d9, 0x8d2a4c8a,
    0xfffa3942, 0x8771f681, 0x6d9d6122, 0xfde5380c,
    0xa4beea44, 0x4bdecfa9, 0xf6bb4b60, 0xbebfbc70,
    0x289b7ec6, 0xeaa127fa, 0xd4ef3085, 0x04881d05,
    0xd9d4d039, 0xe6db99e5, 0x1fa27cf8, 0xc4ac5665,
    0xf4292244, 0x432aff97, 0xab9423a7, 0xfc93a039,
    0x655b59c3, 0x8f0ccc92, 0xffeff47d, 0x85845dd1,
    0x6fa87e4f, 0xfe2ce6e0, 0xa3014314, 0x4e0811a1,
    0xf7537e82, 0xbd3af235, 0x2ad7d2bb, 0xeb86d391,
};

std::uint32_t rotl(std::uint32_t value, std::uint32_t shift) noexcept {
  return (value << shift) | (value >> (32u - shift));
}

std::uint32_t read_u32_le(const std::byte* data) noexcept {
  return std::to_integer<std::uint32_t>(data[0]) |
         (std::to_integer<std::uint32_t>(data[1]) << 8) |
         (std::to_integer<std::uint32_t>(data[2]) << 16) |
         (std::to_integer<std::uint32_t>(data[3]) << 24);
}

void write_u32_le(std::array<std::byte, 16>& out, std::size_t offset, std::uint32_t value) noexcept {
  out[offset + 0] = std::byte(value & 0xffu);
  out[offset + 1] = std::byte((value >> 8) & 0xffu);
  out[offset + 2] = std::byte((value >> 16) & 0xffu);
  out[offset + 3] = std::byte((value >> 24) & 0xffu);
}
} // namespace

MD5Digest md5(std::span<const std::byte> data) {
  std::vector<std::byte> message(data.begin(), data.end());
  const auto bit_len = static_cast<std::uint64_t>(message.size()) * 8u;

  message.push_back(std::byte{0x80});
  while ((message.size() % 64u) != 56u) {
    message.push_back(std::byte{0x00});
  }
  for (int i = 0; i < 8; ++i) {
    message.push_back(std::byte((bit_len >> (8 * i)) & 0xffu));
  }

  std::uint32_t a0 = 0x67452301u;
  std::uint32_t b0 = 0xefcdab89u;
  std::uint32_t c0 = 0x98badcfeu;
  std::uint32_t d0 = 0x10325476u;

  for (std::size_t chunk = 0; chunk < message.size(); chunk += 64) {
    std::array<std::uint32_t, 16> words{};
    for (std::size_t i = 0; i < words.size(); ++i) {
      words[i] = read_u32_le(message.data() + chunk + i * 4);
    }

    std::uint32_t a = a0;
    std::uint32_t b = b0;
    std::uint32_t c = c0;
    std::uint32_t d = d0;

    for (std::uint32_t i = 0; i < 64; ++i) {
      std::uint32_t f = 0;
      std::uint32_t g = 0;
      if (i < 16) {
        f = (b & c) | (~b & d);
        g = i;
      } else if (i < 32) {
        f = (d & b) | (~d & c);
        g = (5u * i + 1u) % 16u;
      } else if (i < 48) {
        f = b ^ c ^ d;
        g = (3u * i + 5u) % 16u;
      } else {
        f = c ^ (b | ~d);
        g = (7u * i) % 16u;
      }

      const auto next = d;
      d = c;
      c = b;
      b += rotl(a + f + k_table[i] + words[g], k_shift[i]);
      a = next;
    }

    a0 += a;
    b0 += b;
    c0 += c;
    d0 += d;
  }

  MD5Digest digest{};
  write_u32_le(digest, 0, a0);
  write_u32_le(digest, 4, b0);
  write_u32_le(digest, 8, c0);
  write_u32_le(digest, 12, d0);
  return digest;
}

} // namespace ed2k::crypto
