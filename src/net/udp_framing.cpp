...
namespace ed2k::net {
namespace {
constexpr std::uint8_t kKadProtocol = 0xE4;
constexpr std::uint8_t kKadPackedProtocol = 0xE5;
}
...
tl::expected<Packet,std::error_code> parse_udp_datagram(std::span<const std::byte> data){
  ...
  if(p.protocol == proto::zlib || p.protocol == kKadPackedProtocol){
    auto dec = zlib_inflate(rest, MAX_PACKET_SIZE);
    if(!dec) return tl::unexpected(dec.error());
    p.protocol = p.protocol == proto::zlib ? proto::eMule : kKadProtocol; // 归一化
