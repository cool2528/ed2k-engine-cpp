...
| **P4c-2 AICH 两级树互操作** | 真实 eMule AICH 两级树（OP_AICHFILEHASHREQ/ANS）+ `aich_hash_bytes` 回两级 + AICHChecker 两级 | 🚧 M1✅ M2✅ M3-M4 待实现（opcode 已对 aMule 确认） |
...
./ed2k_tests.exe                                                          → 216 tests, 213 PASSED, 3 DISABLED, 5 SKIPPED
```

> P4c-2-3/-4（M1 两级 `aich_hash_bytes` + M2 两级 `AICHChecker`）后：3 个 flat-model download AICH 用例 `DISABLED_`（待 M4 per-part 改写，见 G11）；其余 213 全绿，5 skip 预存。
