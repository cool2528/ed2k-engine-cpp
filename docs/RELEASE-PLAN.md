...
| **P4c-3 多源并发 + 续传增强** | raccoon 多源并发 + 块粒度续传 + `peer::Block` u64 + 异步磁盘 I/O + .part.met 接入下载流程 | 🚧 M1✅ M2✅ M3✅（raccoon 并发骨架+多源 e2e+.part.met 续传+异步磁盘卸载）/ M4 待（live + >4GiB 边界，spec✓ Block u64✓） |
...
> P4c-2 全里程碑 + P4c-3 M1 完成后：原 3 个 flat-model download AICH 用例已 re-enable 为两级 per-part（`BlockLevelAICHSingleSource`/`AICHCorruptionRecovers`/`AICHMasterMismatchDegrades`）；P4c-3 M1 新增 3 个多源并发 e2e（`MultiSourceBothFull`/`MultiSourceAggregates`/`MultiSourceSingleSubsetFails`）；全套 224 绿，5 skip 预存（env/live）。
...
- [ ] **P4c-3-8 测试（剩余）**：大文件（>4GiB）边界（M4）。

> **P4c-3 M1+M2+M3 验收 ✅**（2026-07-03）：raccoon 多源并发骨架（S1–S4）+ `.part.met` 续传接入 + 异步磁盘 I/O 卸载全绿，229/229/5。**剩余 v1.0 硬范围**：M4 live 验证 + >4GiB 边界（P4c-3-8 剩余）。
...
- [~] P4c-3 多源并发 + 续传增强完成（raccoon ✅ + Block u64 ✅ + .part.met 接入 ✅ + 异步磁盘 ✅；剩 M4 live 验证 + >4GiB 边界）
