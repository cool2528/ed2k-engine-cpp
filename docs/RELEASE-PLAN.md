...
| 版本（建议） | 里程碑 | 完成定义 | 状态 |
|---|---|---|---|
| **v1.0.0 MVP 下载器** | P4 收口（含 P4c-2/P4c-3）+ 发布工程 | P4c-2 AICH 互操作 + P4c-3 多源并发 + 真实服务器端到端下载（live 绿）+ README/打包/CI + 跨平台构建验证 | 🚧 差 R0-2/R0-3（CI/跨平台构建验证）+ R0-5 版本号升 1.0.0（P4c-2 ✅ + P4c-3 ✅ + R0-1 live ✅） |
...
- [~] **R0-2 CI 流水线**（脚手架已就位 2026-07-03，待首次 push 验证）
  - `.github/workflows/build.yml`：Windows(MSVC, preset `default`)+Ubuntu(GCC, preset `linux`/Ninja) 矩阵，vcpkg manifest 安装，`cmake --preset` + build + `ctest`。live 测试默认 skip（无 `ED2K_LIVE`）。workflow 内 `build/default` 路径已核对：两 preset 的 `binaryDir` 均为 `${sourceDir}/build/default`（`CMakePresets.json` L9/L20），故硬编码路径对双平台均正确——唯一阻塞仍是 vcpkg baseline（见 R0-3）。
  - **目标仓库**：`cool2528/ed2k-engine-cpp`（空 public 仓库，2026-07-04 创建，无分支，待首次 push）。本仓尚未配置 git remote；push 为对外动作，需用户明确授权。
  - ⚠️ 无 Linux/CI 环境本地验证；首次 push 到 GitHub 触发运行，失败则在仓内修 preset/可移植性。
- [~] **R0-3 跨平台构建验证**（静态扫描 + Linux preset 已就位 2026-07-03；本地 WSL 验证进行中 2026-07-04，阻塞于 vcpkg baseline）
  ...
  - 新增 `linux` preset（Ninja 单配置 + `CMAKE_BUILD_TYPE=Debug`）。
  - **本地 WSL 验证进度（2026-07-04，Ubuntu-24.04）**：工具链就位——`g++ 13.3.0` / `cmake 3.28.3` / `ninja 1.11.1`（apt 装）/ `pkg-config 1.8.1`；vcpkg 2026-05-27 clone 到 `/opt/vcpkg` 并 `bootstrap-vcpkg.sh` 完成；源码从 `/mnt/d/workspace/ed2kengine` 复制到 `/root/ed2kengine`（原生 fs，避 /mnt/d I/O 开销，已排除 `build/`/`vcpkg_installed/`/`.git`）。
  - ⚠️ **阻塞点 — vcpkg baseline 不可达**：`vcpkg.json` 的 `builtin-baseline: 15e5f3820f0370f1ba7150853762cec0688cd396` 恰为 Windows 本机 vcpkg 的 HEAD（`D:\tools\vcpkg`，remote = `github.com/microsoft/vcpkg.git`），但在 WSL 内 `git clone --depth 1` + `git fetch --unshallow` 后 `git cat-file -t` 仍报 `Not a valid object name`——疑为该 commit 在上游被 rebase/移除（Windows vcpkg 为较早 fetch 的本地状态，故仍持有）。manifest mode 因 gtest `version>=1.14.0` 需该 baseline 可达，否则 configure 失败。
  - **待解方案（择一）**：① 用完整（非 shallow）`git clone https://github.com/microsoft/vcpkg.git /opt/vcpkg`（非 `--depth 1`）确认该 commit 是否在上游历史——若在，则 unshallow 未完整需重做；若不在，则 ② 将 `vcpkg.json` 的 `builtin-baseline` 更新为可达 commit（当前 HEAD `d87340acc…` 或 tag `2026.06.24`），gtest 仍解析到 ≥1.14.0（最新 1.17.0，与 Windows 一致）。
  - **⚠️ 此 baseline 问题可能同样影响 R0-2 CI**：CI fresh clone vcpkg 同样需该 commit 可达，若上游已无则 CI 首跑必失败——R0-2 push 前必须先确认/修复 baseline（更新为可达 commit 并提交）。
  - **续行命令**（baseline 解决后）：`VCPKG_ROOT=/opt/vcpkg cmake --preset linux && cmake --build --preset linux && ctest --preset linux`（vcpkg manifest 自动装 zlib/spdlog/tl-expected/boost-asio/gtest）。期望 229 绿 + 5 live skip（无 `ED2K_LIVE`）。WSL 环境已就位（vcpkg@/opt/vcpkg、源码@/root/ed2kengine、amuled@:4662 仍在跑），恢复时直接续行。
...
- [ ] R0-2 CI 双平台绿（mock 全过，live skip）— ⚠️ push 前先修 vcpkg baseline（见 R0-3），否则 CI fresh clone 必失败
- [ ] R0-3 Linux + Windows 双平台构建+测试通过 — 本地 WSL 验证进行中，阻塞于 vcpkg baseline 不可达（见 §5 R0-3）
