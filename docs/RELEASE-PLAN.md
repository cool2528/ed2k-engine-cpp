...
- [~] **R0-1 live 真实互操作验证**【v1.0 首要】（3/4 绿，`4268716`）
  ...
  - ⚠️ HighIdSourceCompletes 环境受阻（非代码缺陷，穷举确认）：先试 4 个公网 HighID 源（pdf 33KB/1.3MB/2.4MB + mkv 264MB，TCP 可达性确认）全部失败——1 超时/1 RST/2 connection_closed。recv 诊断显示对端在**任何协议交换前**关闭（未发任何 eMule 帧，HELLO 从未被回应）。随后写 `ProbeReachableHighIdSource` 探针（TEMP，已移除）批量握手 12 个跨多文件的 HighID 源：**12/12 全失败**（8 connection_closed + 4 connect_failed），0 个回 HELLOANSWER。结论确凿：2026 公网 eMule 源对"冷"直连（非服务器 callback 引荐、未知 IP）一律在协议交换前关闭/拒连。P2P framing+HELLO 已对照 aMule 校验（proto 0xE3+OP_HELLO=0x01/ANS=0x4C，与 mock+真实服务器登录同 framing 路径，live 服务器登录即用此路径且绿）。secure ident（OP_PUBLICKEY_REQ）/协议混淆未实现但非致因——对端未读 HELLO 即关闭。下载路径本身已由 225 mock e2e 锁定（含 EndToEndSingleSource/Resume/BlockCorrupt）。**转绿唯一途径**：用户提供可靠源（自建 HighID eMule 实例或长期在线、接受冷连的源），否则公网无法验证。
...
- [~] **R0-3 跨平台构建验证**（静态扫描 + Linux preset 已就位 2026-07-03，待 CI/Linux 机器验证）
  ...
  - 目标：双平台 `ed2k_tests` 全绿（live skip）——待 R0-2 CI 首跑或 Linux 机器确认。
...
- [~] R0-1 live 4 测试绿（3/4：Login/Search/GetSources ✅；HighIdSourceCompletes 环境受阻——12 个公网 HighID 源穷举探针 0/12 回 HELLO，协议交换前关闭，非代码缺陷，见 §6 R0-1；待用户提供可靠源）
...
- [x] P4c-3 多源并发 + 续传增强完成（raccoon ✅ + Block u64 ✅ + .part.met 接入 ✅ + 异步磁盘 ✅ + >4GiB 边界 ✅；M4 live=HighIdSourceCompletes 同 R0-1 环境受阻）
- [x] 既有 mock 回环全绿不回归（225 绿 + 4 live skip，见 §3.1）
