...
| D4 | 发布同步 | ✅ 已完成：`main` 推送到 `5ca81d2`，tag `v2.2.0` 指向该提交，GitHub Release 已发布 | v2.2.0 |
...
| 2026-07-06 | **v2.2.0 发布完成**：运行时版本、`project(VERSION)`、`vcpkg.json`、README、CHANGELOG 与 RELEASE-PLAN 当前版本同步到 `2.2.0`；新增 `Smoke.VersionMatchesRelease` 锁定 `ed2k::version()` 与发布号一致。验证：focused `Smoke.*` 2/2 pass；Windows build `ed2k_tests`/`ed2k-tool` + full ctest：453 total，444 pass + 9 live skip，0 fail；WSL Linux `wsl -u root` build + `ctest --preset linux`：453 total，444 pass + 9 live skip，0 fail；`git diff --check` 仅 CRLF warning；CI run `28765725134`：ubuntu-latest + windows-2022 均通过。发布：`main` 推送到 `5ca81d2`，annotated tag `v2.2.0` 已推送，GitHub Release（非 draft、非 pre-release）发布：https://github.com/cool2528/ed2k-engine-cpp/releases/tag/v2.2.0 | `5ca81d2` + tag `v2.2.0` |
