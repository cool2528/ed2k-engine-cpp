...
- [~] **R0-1 live 真实互操作验证**【v1.0 首要】（3/4 绿，`4268716`）
  ...
  - ⚠️ HighIdSourceCompletes 环境受阻（非代码缺陷）：试 4 个公网 HighID 源（pdf 33KB/1.3MB/2.4MB + mkv 264MB，TCP 可达性确认）全部失败——1 超时/1 RST/2 connection_closed。recv 诊断显示对端在**任何协议交换前**关闭（未发任何 eMule 帧，HELLO 从未被回应）。根因：2026 公网 eMule 源稀疏，对"冷"直连（非服务器 callback 引荐）拒连/离线/端口转发到空。P2P framing+HELLO 已对照 aMule 校验（proto 0xE3+OP_HELLO=0x01/ANS=0x4C，与 mock+真实服务器登录同 framing 路径）。secure ident（OP_PUBLICKEY_REQ）未实现但非致因——对端未发任何帧即关闭。下载路径本身已由 225 mock e2e 锁定（含 EndToEndSingleSource/Resume/BlockCorrupt）。需可靠源（自建 HighID eMule 实例或长期在线源）方能转绿。
...
- [~] R0-1 live 4 测试绿（3/4：Login/Search/GetSources ✅；HighIdSourceCompletes 环境受阻——4 公网源协议交换前关闭，非代码缺陷，见 §6 R0-1；待可靠源）
