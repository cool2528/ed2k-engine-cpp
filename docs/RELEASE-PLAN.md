...
| **P4c-3 多源并发 + 续传增强** | raccoon 多源并发 + 块粒度续传 + `peer::Block` u64 + 异步磁盘 I/O + .part.met 接入下载流程 | 🚧 M1✅（raccoon 并发骨架+多源 e2e 绿）/ M2/M3/M4 待（spec✓ Block u64✓） |
...
- [ ] **P4c-3-8 测试（剩余）**：`.part.met` 续传 round-trip（M2）+ 大文件（>4GiB）边界（M4）。

> **P4c-3 M1 验收 ✅**（2026-07-03）：raccoon 多源并发骨架（S1–S4）全绿, 219/224/5。M1 = P4c-3-3/4/5/8(mock 部分)。**剩余 v1.0 硬范围**：M2 `.part.met` 续传接入（P4c-3-6）、M3 异步磁盘 I/O（P4c-3-7）、M4 live 验证 + >4GiB 边界（P4c-3-8 剩余）。
