...
| **P4c-2 AICH 两级树互操作** | 真实 eMule AICH 两级树（OP_AICHFILEHASHREQ/ANS）+ `aich_hash_bytes` 回两级 + AICHChecker 两级 | 🚧 spec✓ M1-M4 待实现（opcode 已对 aMule 确认） |
...
./ed2k_tests.exe                                                          → 214 tests, 209 PASSED, 5 SKIPPED
```
...
- [ ] **P4c-2-3 `aich_hash_bytes` 回两级**（M1）：扁平 lone-child Merkle → 两级平衡二叉树（顶层 PARTSIZE base → 底层 EMBLOCKSIZE base），分裂规则对照 aMule `FindHash`；测试向量覆盖单块/单 part/多 part。
- [ ] **P4c-2-4 `AICHChecker` 两级校验**（M2）：`verify_block(part_index, block_in_part, data, proof)` 按 hash identifier 定位；构造 `(AICHHash, file_size)`。
- [ ] **P4c-2-5 线协议编解码 + C2CConnection**（M3）：改 `AICHREQUEST/ANSWER`=0x9B/0x9C、新增 `AICHFILEHASHREQ/ANS`=0x9E/0x9D、协议层改 `proto::eMule`(0xC5)；重写 `encode_aich_request`/`decode_aich_answer` 对齐 V2 帧（file_hash→part_index→master_hash）；加 `request_aich_master_hash`。
- [ ] **P4c-2-6 `peer_worker` 启用 AICH + per-part 块**（M4）：`BlockAllocator`/`PartFile` 改 per-part 块分解（块不跨 part 边界）；`peer_worker` AICH 分支改 part-local 索引 + master hash 交换；删除 `num_blocks<=65535` 守卫。
- [ ] **P4c-2-7 测试**（M4）：mock 两级 AICH round-trip（corruption→recover）+ master hash 不匹配降级 + 多 part proof path；live（若有支持 AICH 的真实源）。
...
| 2026-07-03 | P4c-2-2 完成：两级 AICH 实现计划（`docs/superpowers/plans/2026-07-03-ed2k-p4c2-aich-two-level-impl-plan.md`），3 阶段 S1（M1+M2 合并：`aich_checker_test` 用 `aich_hash_bytes` 当 oracle 故耦合）/S2（M3 线协议）/S3（M4 per-part 块，P4c-2→P4c-3 块模型门）+ G1-G10 陷阱 + 既有测试破改分析（`FlatMerkleTwoChunkMatchesManual` 必破、3 块巧合同根）。回填 `13fa676`（本提交） |
