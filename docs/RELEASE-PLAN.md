...
| 2026-07-03 | R0-4 完成（README + 用户文档，`<本提交>`）：`README.md`（简介/协议覆盖/构建 vcpkg+CMake/CLI/架构 single-io_context+raccoon+异步磁盘/Asio gotcha/关键参数/live 门）、`docs/USAGE.md`（7 CLI 子命令详解+示例+退出码+.part.met 续传）、`docs/API.md`（公共 API 按模块概览+并发契约）、`CHANGELOG.md`（Keep a Changelog，P4c-1/2/3+0.1.0）。§4.1 CHANGELOG/release-notes 项勾除 | R0-4 |
