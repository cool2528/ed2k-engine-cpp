...
>
> **最近更新**：2026-07-06 — P9-7 跨客户端续传 + live 完成：`PartFile` 对 `.part` 输出使用 aMule sibling sidecar（`001.part` -> `001.part.met`），mock handoff 测试绿；真实 aMule 2.3.3 -> 本节点续传完成（hash `114b21c63a74b6ca922291a11177dd5c`），反向本节点 `.part/.part.met` -> aMule 续传完成且 `LiveUpload.AcceptsLocalPeerUploadSession` 通过。
...
- [ ] **R1-4（M2/P1）结构 + 模式**：`Download::run`/`peer_worker` 拆阶段 ✅ S4（`a2fe93e`）+ 公共 API PIMPL ✅ S5（本地提交待记录）+ Observer RAII Subscription。
