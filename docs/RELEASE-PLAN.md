...
- [~] **R0-2 CI 流水线**（脚手架已就位 2026-07-03，待首次 push 验证）
  - `.github/workflows/build.yml`：Windows(MSVC, preset `default`)+Ubuntu(GCC, preset `linux`/Ninja) 矩阵，vcpkg manifest 安装，`cmake --preset` + build + `ctest`。live 测试默认 skip（无 `ED2K_LIVE`）。
