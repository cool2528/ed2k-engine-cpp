...
>
> **最近更新**：2026-07-05 — P7-4 TCP 混淆握手 + 帧加密完成：新增 aMule basic peer obfuscation `EncryptedStreamSocket`（clear marker/random key、MD5(user-hash + role magic + random)、RC4 discard 1024、握手后帧透明加解密）；下一实施项为 P7-5 UDP + 服务器混淆 + 协商 fallback。
...
| **v2.0** | P7 协议混淆 | MD5/RC4 + TCP/UDP/服务器混淆 + 握手协商 + fallback（spec/plan 已起草） | 🚧 进行中：P7-1..P7-4 ✅；P7-5 UDP + 服务器混淆 + 协商 fallback 下一项（[spec](superpowers/specs/2026-07-04-ed2k-p7-obfuscation-design.md)/[plan](superpowers/plans/2026-07-04-ed2k-p7-obfuscation-impl-plan.md)） |
...
| **P7 混淆加密** | L6：MD5/RC4 自带、TCP/UDP/服务器混淆、握手协商+fallback | 兼容开启 obfuscation 的网络 | 🚧 进行中 | P7-1..P7-4 ✅；P7-5 下一项 |
