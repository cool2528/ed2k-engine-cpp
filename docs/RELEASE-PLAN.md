...
> P4c-2-3/-4/-5（M1 两级 `aich_hash_bytes` + M2 两级 `AICHChecker` + M3 AICH 线协议 V2 帧）后：3 个 flat-model download AICH 用例 `DISABLED_`（待 M4 per-part 改写，见 G11）；其余 210 全绿，5 skip 预存。
