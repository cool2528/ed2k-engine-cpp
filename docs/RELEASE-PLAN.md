...
>
> **最近更新**：2026-07-06 — v2.2.0 发布准备：运行时版本、CMake/vcpkg manifest、README 与 CHANGELOG 同步到 `2.2.0`；Windows/WSL full gates 均 453 total、444 pass + 9 live skip、0 fail；下一步执行最终 push/tag/release。
...
| D4 | 发布同步 | R1/P5-P9 本地提交已完成；Windows/WSL gates 已全绿，远端 push/tag/release 为下一步 | 本轮发布 |
...
| 2026-07-06 | **v2.2.0 发布准备**：运行时版本、`project(VERSION)`、`vcpkg.json`、README、CHANGELOG 与 RELEASE-PLAN 当前版本同步到 `2.2.0`；新增 `Smoke.VersionMatchesRelease` 锁定 `ed2k::version()` 与发布号一致。验证：focused `Smoke.*` 2/2 pass；Windows build `ed2k_tests`/`ed2k-tool` + full ctest：453 total，444 pass + 9 live skip，0 fail；WSL Linux `wsl -u root` build + `ctest --preset linux`：453 total，444 pass + 9 live skip，0 fail；`git diff --check` 仅 CRLF warning。最终 push/tag/release 下一步执行 | 本地提交 |
