...
| 2026-07-03 | R0-2/R0-3 脚手架（CI + Linux preset，`<本提交>`）：`.github/workflows/build.yml`（Windows MSVC preset `default` + Ubuntu GCC preset `linux` 矩阵，vcpkg manifest，ctest live 默认 skip）；`CMakePresets.json` 新增 `linux` preset（Ninja 单配置）；静态扫描确认库/CLI 无 Winsock/`_WIN32`/Windows API/反斜杠路径（全经 Boost.Asio；`_WIN32_WINNT` 已 `$<PLATFORM_ID:Windows>` 守卫）。⚠️ 无 Linux/CI 环境本地验证，待首次 push 触发 | R0-2/3 |
