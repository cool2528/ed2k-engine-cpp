...
>
> **最近更新**：2026-07-05 — P5-8 SourceExchange 进展（本地提交 `cb04634`）：`KnownFile` 增运行时 SX2 源列表，`UploadSession` 可返回非空 `ANSWERSOURCES2`；Windows 本地 285 测试 278 pass + 7 live skip。真实 aMule live 仍卡在 aMule `Total sources=0` 不回 SX2 / source-extended link 不入源列表。
