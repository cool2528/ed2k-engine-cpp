...
> P4c-2 全里程碑（M1 两级 `aich_hash_bytes` + M2 两级 `AICHChecker` + M3 AICH 线协议 V2 帧 + M4 per-part 块+peer_worker 集成+mock e2e）完成后：原 3 个 flat-model download AICH 用例已 re-enable 为两级 per-part（`BlockLevelAICHSingleSource`/`AICHCorruptionRecovers`/`AICHMasterMismatchDegrades`）；全套 221 绿，5 skip 预存（env/live）。
