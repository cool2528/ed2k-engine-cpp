...
- [x] **R0-3 跨平台构建验证**（✅ 2026-07-04 CI 双平台绿 + 本地 Windows 绿）
  ...
  - **本地 WSL Linux 验证**：vcpkg 逐 Boost 子包下载源码，WSL2 网络/IO 慢（~40min+ 仍 header-only），不适合迭代；**CI ubuntu-latest 为权威 Linux 验证**（run `28695894770` ✅ 234/229/5），满足 R0-3 跨平台 DoD。
  - 目标：双平台 `ed2k_tests` 全绿（229 pass + 5 live skip）。✅ 达成（CI 双平台 + 本地 Windows）。
...
| 2026-07-04 | **🎉 v1.0.0 发布就绪**：CI run `28695894770`（`cf8d4e7`）**双平台全绿**——Windows-2022 + Ubuntu-latest 各 234 测试 229 pass + 5 live skip，0 fail。R0-2（CI）✅ + R0-3（跨平台）✅。**R0-5 版本号升 1.0.0**：`version.cpp`/`CMakeLists.txt`/`vcpkg.json`/`README.md`/`CHANGELOG.md` 全部 0.1.0→1.0.0。§6 发布检查清单全 ✅。打 tag `v1.0.0` + GitHub Release | `cf8d4e7`→(release) |
