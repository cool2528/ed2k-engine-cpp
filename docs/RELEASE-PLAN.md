...
> - 本文是**跨阶段、滚动更新**的总进度表。各阶段设计/实现计划为已冻结的内部历史快照（不随仓库发布）。
>
> **最近更新**：2026-07-15 — aMule optional 上传 live gate 首次通过。根因：① harness 源地址曾解析为 Windows vEthernet 网关（`hostname -I` 首字段不稳定）；② aMule 仅在 `theApp->IsConnected()` 时才连接下载源（aMule 2.3.3 `PartFile.cpp` 门控），隔离 harness 中源永不被连接。修复：eth0 全局地址确定性推导 + 本地 eD2k 登录 stub（`LOGINREQUEST`→`IDCHANGE` HighID）+ `Add` 后源登记验证 + `Ed2kServersUrl=disabled` 保持离线。optional 全绿（4 混淆断言 + aMule 54s 完整下载 19,456,000 字节上传证据），required 复验 5/5 绿；Windows 535/535、Linux 524/524，0 fail。
...
| 当前版本 | `2.4.0`（`CMakeLists.txt` `project(VERSION)`，`include/ed2k/version.hpp.in` 据此生成） |
...
| **v1.1** | P5 上传/分享/信用 | 共享索引 + 服务器发布 + 上传会话 + 上传队列+限速 + 信用系统 + SourceExchange v2 + 评论/评分（spec/plan 已起草） | ✅ P5 完成：M1/S1 + M2/S2-S4 上传/发布 + M3/S5-S7 上传队列/限速/信用 + M4/S8 SourceExchange/评论 + P5-8 aMule live 绿 |
| **v1.2** | P6 KAD | Kad 路由表 + bootstrap + Kad2 UDP + 搜索/取源/发布 + buddies + notes（spec/plan 已起草） | ✅ P6 完成：P6-1..P6-7 ✅，公网 Kad bootstrap/search live 绿 |
| **v2.0** | P7 协议混淆 | MD5/RC4 + TCP/UDP/服务器混淆 + 握手协商 + fallback（spec/plan 已起草） | ✅ P7 完成：P7-1..P7-5 ✅ |
| **v2.1** | P8 客户端基础设施 | IPFilter + Preferences + Statistics + Category + FriendList + ClientList + Proxy + HTTPThread + .emulecollection + preview + scheduler + **ChatRelay（OP_MESSAGE 中继）**（spec/plan 已起草） | ✅ P8 完成：P8-1..P8-7 ✅ |
| **v2.2** | P9 eD2k 协议完备性 | 服务器 UDP 完备 + 服务器列表交换 + MuleInfo + AICH 共享端 + 压缩块上传 + aMule-faithful .part.met（跨客户端续传）+ 大文件边界（spec/plan 已起草） | ✅ P9 完成：P9-1..P9-7 ✅，跨客户端 aMule ↔ 本节点 live 绿 |
...
| **P8 客户端基础设施** | L7：IPFilter、Preferences、Statistics、Category、FriendList、ClientList、Proxy、HTTPThread、.emulecollection、preview、scheduler、ChatRelay | 完整客户端管理能力 | ✅ 完成 | P8-1..P8-7 ✅ |
| **P9 eD2k 协议完备** | L8：服务器 UDP 完备、服务器列表交换、MuleInfo、AICH 共享端、压缩块上传、aMule-faithful .part.met、大文件边界 | aMule 协议级全功能对等 | ✅ 完成 | P9-1..P9-7 ✅；跨客户端 aMule ↔ 本节点 live 绿 |
...
```
Windows Debug → 535 total, 519 pass + 16 live skip, 0 fail（2026-07-15）
Linux Debug   → 524 total, 508 pass + 16 live skip, 0 fail（2026-07-15）
（Release/install/consumer 沿用 2026-07-12 四组全绿证据：Windows/Linux Debug/Release + install/consumer smoke）
```

> 默认 mock 回环全绿；16 个 live-gated 测试默认 skip（新增 `LiveServerStub` harness 工具项）。**2026-07-15 optional upload hard gate 首次通过**：aMule 经本地 eD2k 登录 stub 连接后，从注入源完整下载 19,456,000 字节（上传证据 54s 完成），required 模式复验 5/5 绿。此前 `Total sources: 0` 的根因有二：① harness 源地址取 `hostname -I` 首字段，2026-07-12 运行解析为 Windows vEthernet 网关 `172.28.192.1`（WSL 内不可达）；② aMule 仅在 `theApp->IsConnected()`（已连 eD2k/Kad）时才对下载源调用 `AskForDownload`（aMule 2.3.3 `PartFile.cpp` DS_NONE 分支门控），完全隔离的 harness 中源被登记但永不连接。公网 probe 的结论（server-list 获取失败、5 个静态 fallback 不可达）维持 2026-07-12 记录不变。
...
- **L3 下载引擎**：C2C 握手/hashset/filestatus/逐块下载、PartFile（part-MD4 校验+磁盘写+gap 续传+per-part block bitmap）、BlockAllocator（per-part 块分发）、AICHChecker（**两级 Merkle 标识符重建**，master-hash 协商降级）、MultiSourceDownload（raccoon 多源并发）、InboundListener（入站 C2C acceptor）、LowID 回调（OP_CALLBACKREQUEST + acceptor-mode 握手）
- **L4 上传/分享/信用**：KnownFileDB、known.met/known2.met、服务器发布、UploadSession/UploadQueue/限速、ClientCredits、SourceExchange v2、评论/评分
- **L5 KAD**：Kad2 routing/bootstrap、UDP 编解码、关键词/源/notes 搜索与发布、firewalled/buddy/callback、CLI Kad 命令
- **L6 混淆**：自带 MD5/RC4、TCP encrypted stream、UDP/server obfuscation、协商位与 fallback
- **L7 客户端基础设施**：IPFilter、Preferences、Statistics、Category、FriendList、ClientList、Proxy、HTTPThread、collection、preview、scheduler、ChatRelay
- **L8 协议完备**：服务器 UDP v2/ident、服务器列表交换、MuleInfo、AICH 共享端、压缩块上传、aMule-faithful `.part.met`、大文件边界
- **CLI**：见附录 A，覆盖 hash/server/Kad/publish/comment/config/stats/collection/scheduler/download 等命令

### 3.3 已知技术债 / 未接线项

| # | 项 | 现状 | 归属 |
|---|---|---|---|
| D1 | C++23 迁移 | `std::expected`/库版 `std::format` 与 C++23 baseline 暂缓，避免在协议层刚稳定后扩大工具链风险 | R1-6 ⏭️ |
| D2 | LowID/NAT 完整 live 环境 | listener + callback 单测覆盖；NAT 后全路径仍需公网/路由环境验证 | 后续（NAT 穿透/UPnP） |
| D3 | 公网 HighID live 源稳定性 | 公网 peer 常过滤云 IP；重复端到端验证以本地 aMule 2.3.3 peer 为可靠源 | live 运维 |
| D4 | 发布同步 | ✅ 已完成：release tag `v2.2.0` 指向 `5ca81d2`，GitHub Release 已发布；release 记录已追加到 `main` | v2.2.0 |
...
- [x] **P4c-3-1 设计 spec**（✅ 2026-07-03）：四里程碑 M1（raccoon 骨架 + `peer::Block` u64 + 共享 BlockAllocator）/ M2（`.part.met` 续传接入）/ M3（异步磁盘 I/O）/ M4（live + >4GiB 边界）。并发原语 = 同步 `BlockAllocator::next_block()` 块分发 + `asio::experimental::channel` 完成信号，**禁用 `condition_variable`** 以避免单线程 io_context 死锁（破除 `download.cpp:190-191` 注释约束）。AICH 路径不动（属 P4c-2）。
- [x] **P4c-3-2 实现计划**（✅ 2026-07-03）：M1 拆 4 个 green-deployable 阶段（S1 `fetch_hashset`+`SharedState` 脚手架 / S2 共享状态 `peer_worker`+setup 顺序 / S3 raccoon 并发+源耗尽处理 / S4 多源 mock e2e）+ 10 项陷阱清单 + 每阶段测试门。M2/M3/M4 待 M1 落地另起。
...
- [x] **P5-8 测试 + live**：mock 上传回环 + live（本地 aMule peer 互传源/上传）完成。`72793b3` 补齐 eMule HELLO SX2/multipacket 能力，`C2CConnection::request_sources2` 改走 aMule 可靠的 `OP_MULTIPACKET` 嵌套 `OP_REQUESTSOURCES2(version=4/options=0)`，`UploadSession` 支持 `OP_MULTIPACKET(_EXT)` 下的文件名/文件状态/AICH/SX2 子操作，`decode_req_filename_answer` 兼容 aMule `u16` 字符串。live harness 使用两个临时身份：holder 先 `SETREQFILEID`+`STARTUPLOADREQ` 进入 aMule 上传列表，再 `REQUESTFILENAME` 清空 part-status 陷阱，requester 随后请求 SX2 并断言返回 holder 源。验证：`LiveUpload.SourceExchange2WithLocalPeer` 对 WSL aMule 2.3.3 真实 live 绿；Windows `cmake --build build/default --config Debug --target ed2k_tests ed2k-tool -- /m` + `ctest -C Debug -j8 --output-on-failure`：288 测试 281 pass + 7 live skip，0 fail。
...
- [x] **P6-1 设计 spec**（✅ 已起草）：Kad2 128-bit + 路由表 + Kad2 UDP 帧字节级 + M1-M4 + G1-G9。
- [x] **P6-2 实现计划**（✅ 已起草）：S1-S7 + 测试门 + 破改分析。
- [x] **P6-3 KadID + 路由表 + nodes.dat**（✅ 2026-07-05，M1/S1，`e329bfa`）：`ed2k::kad::KadID` 128-bit + MD4(userhash+nonce) 生成、XOR 距离/比较/bit 序；`Contact` + `KBucket`(k=10, LRU update) + `RoutingZone`/`RoutingTable`（满桶仅本机 ID 范围分裂，稀疏 sibling 合并）+ 最近 k 查找；compact `nodes.dat` `[magic:KAD2][count][kadid+ip+udp+tcp+version]` round-trip。测试 +9：KadID、bucket LRU、split/no-split、merge、closest lookup、nodes.dat success/error。验证：Windows + WSL Linux 各 297 测试 290 pass + 7 live skip，0 fail。
- [x] **P6-4 Kad2 UDP + Hello + bootstrap**（✅ 2026-07-05，M2/S2-S3，`4666a1c`）：按 aMule 当前源码修正 Kad2 常量（`OP_KADEMLIAHEADER=0xE4`，HELLO `0x11/0x19`，REQ/RES `0x21/0x29`）与 Hello layout（`kadid+tcp_port+version+tagCount+tags`，非旧 spec 草稿的 8B clear verify key）；新增 `messages` 编解码、`KadNetwork` loopback HELLO/REQ/RES、receiver-check KadID 防伪造、seed bootstrap 收敛。测试 +6：字节布局/round-trip/畸形拒绝、HELLO 路由更新、REQ closest+wrong receiver check、bootstrap learn contacts。验证：Windows + WSL Linux 各 303 测试 296 pass + 7 live skip，0 fail。**边界**：aMule UDP sender/receiver verify key 属 encrypted datagram/P7 obfuscation 层，本阶段不伪造明文字段。
- [x] **P6-5 搜索/取源/发布**（✅ 2026-07-05，M3-M4/S4-S5，`a2e3ae6`）：按 aMule 当前源码实现 Kad2 `SEARCH_KEY/SOURCE/NOTES_REQ` + `SEARCH_RES`、`PUBLISH_KEY/SOURCE/NOTES_REQ` + `PUBLISH_RES` 编解码；新增 `KadIndexed`（关键词→文件、文件→源、文件→notes，TTL 过期 + answer_id 去重 + size 过滤）与 `KadNetwork::{publish_keyword,publish_source,publish_notes,search_keyword,find_sources,search_notes}`。搜索对提供的 peer 集合聚合去重，并在空结果时经 `KADEMLIA2_REQ` bounded iterative 扩展 closest contacts 后继续收敛。测试 +11：搜索/发布 wire round-trip、notes wire、TTL/size/dedupe、publish→search loopback、closest-node 发现 indexer、跨 peer 源去重、notes publish/search。验证：Windows + WSL Linux 各 314 测试 307 pass + 7 live skip，0 fail。
- [x] **P6-6 Firewalled + buddies**（✅ 2026-07-05，M4/S6，`71243be`）：按 aMule 当前源码修正旧 spec 草稿的 firewalled opcode：Kad2 `KADEMLIA_FIREWALLED2_REQ=0x53`、legacy `KADEMLIA_FIREWALLED_REQ/RES/ACK=0x50/0x58/0x59`、buddy `KADEMLIA_FINDBUDDY_REQ/RES=0x51/0x5A`、callback `KADEMLIA_CALLBACK_REQ=0x52`、`KADEMLIA2_FIREWALLUDP=0x62`。新增 firewall UDP result、TCP firewalled check、find buddy、callback 编解码；`KadNetwork` 增 UDP firewall state、observed external IP、buddy active/inactive state、buddy request/response、callback event 记录。测试 +8：wire layout/malformed、UDP firewall open/firewalled、TCP firewall response、buddy 建立/失活、Kad callback。验证：Windows + WSL Linux 各 322 测试 315 pass + 7 live skip，0 fail。
- [x] **P6-7 集成 + CLI + live**（✅ 2026-07-05，S7，`6deabe6`）：`MultiSourceDownload::Builder` 可注入 `kad::KadNetwork`，服务器返回源不足时追加 Kad `find_sources` 的直连 HighID 源；CLI 新增 `kad-bootstrap`、`kad-search`、`kad-find-sources`、`kad-publish` 并同步 README/USAGE/API。按 aMule 修正 Kad UInt128 wire/crypto 字节序、Kad packed UDP inflate、public `nodes.dat` 变体、Kad search closest-node/SEARCHTOLERANCE 策略，以及 tag codec `BLOB=0x07`/`BSOB=0x0A`/`UINT64=0x0B`/`STR1..STR22`。live 证据：`kad-bootstrap %TEMP%\nodes_https_upd_emule_security_org_nodes_dat.dat` → loaded=191/routing=49；`kad-search ... ubuntu` → 470 results。验证：Windows `ctest --test-dir build/default -C Debug -j8 --output-on-failure`：352 测试 344 pass + 8 live skip，0 fail；WSL Linux `cmake --build --preset linux` + `ctest --preset linux`：352 测试 344 pass + 8 live skip，0 fail。
...
- [x] **P7-1 设计 spec**（✅ 已起草，2026-07-05 checklist sync）：MD5/RC4 + TCP/UDP/服务器混淆握手字节级 + M1-M4 + G1-G8。
- [x] **P7-2 实现计划**（✅ 已起草，2026-07-05 checklist sync）：S1-S6 + 测试门 + 破改分析。
- [x] **P7-3 MD5/RC4 自带实现**（✅ 2026-07-05，M1/S1，`bd74243`）：复用既有 `ed2k::crypto::md5` RFC1321 向量；新增共享 `ed2k::crypto::RC4`（KSA/PRGA、buffer/byte 处理、discard、keystream 续接），并替换 Kad UDP obfuscation 私有 RC4，保持 aMule 对照测试字节不变。测试 +4：RC4 已知向量、跨 `process` 续接、`process_byte` 等价、`discard` 跳过 keystream。验证：Windows `cmake --build build/default --config Debug --target ed2k_tests ed2k-tool -- /m` + `ctest --test-dir build/default -C Debug -j8 --output-on-failure`：356 测试 348 pass + 8 live skip，0 fail；WSL Linux（sanitized PATH + `VCPKG_ROOT=/opt/vcpkg`）`cmake --build --preset linux` + `ctest --preset linux`：356 测试 348 pass + 8 live skip，0 fail。
- [x] **P7-4 TCP 混淆握手 + 帧加密**（✅ 2026-07-05，M2/S2-S3，`84ad587`）：新增 `net::EncryptedStreamSocket`，按 aMule basic peer obfuscation 完成 clear marker/random-key 前缀、`MD5(user_hash || role_magic || random_key_part)` 派生、RC4 discard 1024、sync/method/padding 握手与握手后 eD2k frame 透明加解密；测试覆盖 aMule 确定性握手向量、plain marker 拒绝、错误 magic 拒绝、加密 frame round-trip、双向多帧 RC4 续接。验证：Windows `cmake --build build/default --config Debug --target ed2k_tests ed2k-tool -- /m` + `ctest --test-dir build/default -C Debug -j8 --output-on-failure`：362 测试 354 pass + 8 live skip，0 fail；WSL Linux（sanitized PATH + `VCPKG_ROOT=/opt/vcpkg`）`cmake --build --preset linux` + `ctest --preset linux`：362 测试 354 pass + 8 live skip，0 fail；`git diff --check` exit 0。
- [x] **P7-5 UDP + 服务器混淆 + 协商 fallback**（✅ 2026-07-05，M3-M4/S4-S6，`0440e19`）：新增 aMule server UDP obfuscation codec（8 字节 header、`MD5(base_key || direction_magic || random_key_part)`、RC4、sync `0x13EF24D5`、client/server magic `0x6B/0xA5`、确定性向量）；`server.met` 解析/写出 obfuscation tags（UDP flags/key/key-ip、TCP/UDP obf port）；`UdpServerConnection` 支持 obfuscated UDP port 探测、响应解密与 timeout fallback plain；HELLO misc-options2 bits 7/8/9 编解码；`ServerTarget` 透传 obfuscation 元数据。验证：Windows `cmake --build build/default --config Debug --target ed2k_tests ed2k-tool -- /m` + `ctest --test-dir build/default -C Debug -j8 --output-on-failure`：372 测试 364 pass + 8 live skip，0 fail；WSL Linux root `build/linux` configure/build + `ctest --test-dir build/linux -j8 --output-on-failure`：372 测试 364 pass + 8 live skip，0 fail；`git diff --check` exit 0（仅 CRLF warnings）。live obfuscation server/peer 未在本切片实际跑通，仍由门控 live 测试覆盖后续环境验证。
...
- [x] **P8-1 设计 spec**（✅ 已起草，2026-07-05 checklist sync）：IPFilter/Preferences/Statistics/Category/FriendList/ClientList/Proxy/HTTPThread/collection/preview/scheduler + ChatRelay + M1-M5 + G1-G10。
- [x] **P8-2 实现计划**（✅ 已起草，2026-07-05 checklist sync）：S1-S8 + 测试门 + 破改分析。
- [x] **P8-3 IPFilter**（✅ 2026-07-05，M1/S1，`a4cf0e3`）：新增 `ed2k::infra::IPFilter`，支持 `ipfilter.dat` range 行（`start,end,level,name`）、CIDR 行、注释/CRLF/BOM、level `> threshold` 过滤语义、按 start 排序 + prefix max-end 索引的 lookup（覆盖 broad-range overlap）；新增 `errc::ip_filtered`。连接前过滤接入 `net::Connection`、`ServerConnection`、`C2CConnection`、`Download`/`MultiSourceDownload` peer/Kad source、`KadNetwork` UDP outbound、`UploadSession` accepted remote pre-HELLO；CLI 新增 `ed2k-tool ipfilter <ipfilter.dat> [--block-check:ip] [--level:n]`。测试 +10：parser/range/CIDR/level/overlap/malformed、CLI block-check、server/C2C/Kad/upload boundary reject。验证：focused `IPFilter.*:ServerConnection.IpFilter*:C2CConnection.IpFilter*:KadNetwork.IpFilter*:UploadSession.IpFilter*:CliIPFilter.*` 10/10 pass；Windows build + ctest 382 测试 374 pass + 8 live skip，0 fail；WSL Linux root `build/linux` configure/build + ctest 382 测试 374 pass + 8 live skip，0 fail；`git diff --check` exit 0（仅 CRLF warnings）。按用户要求仅本地提交，未 push/未触发 CI。
- [x] **P8-4 Preferences + Statistics**（✅ 2026-07-05，M2-M3/S2-S3，`291c87e`）：新增 `ed2k::infra::Preferences`，覆盖端口、带宽、slot/连接/源数、超时、server/Kad/IPFilter/obfuscation/update/share hidden 开关、核心路径与 shared dirs，使用内部 `E2PF` taglist 格式 round-trip（aMule-faithful import 留 P9）；CLI 新增 `ed2k-tool config <preferences.dat> [--set:key=value]`。新增 `ed2k::infra::Statistics`，维护 session + cumulative uploaded/downloaded bytes、server connections、failed connections、Kad packets、sources、completed files，持久化 cumulative-only `E2TA` 格式，`async_flush` 卸载到 disk executor 后回到 caller executor；CLI 新增 `ed2k-tool stats <statistics.dat>`。测试新增 9 个 focused 用例覆盖默认值、round-trip、bad magic、session/cumulative 计数、cumulative 持久化、异步 flush 不阻塞、CLI config/stats。验证：focused `Preferences.*:Statistics.*:CliConfig.*:CliStats.*` 9/9 pass；Windows build + ctest 391 测试 383 pass + 8 live skip，0 fail；WSL Linux root `build/linux` configure/build + ctest 391 测试 383 pass + 8 live skip，0 fail；`git diff --check` exit 0（仅 CRLF warnings）。按用户要求仅本地提交，未 push/未触发 CI。
- [x] **P8-5 Category + FriendList + ClientList**（✅ 2026-07-05，M4/S4-S5，`2e37a2c`）：新增 `ed2k::infra::Category`/`CategoryList`，内部 `E2CT` 格式 round-trip，Preferences 持久化 `categories` blob，按 regex 列表顺序计算下载完成归档路径。新增 `ed2k::infra::FriendList`，内部 `E2FR` 格式 round-trip，支持 user hash + 可选 KadID + name + friend-slot 标志；`UploadQueue` 可注入 FriendList，排队时 friend-slot peer 优先于普通 peer（同组继续沿用 ClientCredits score/FIFO）。新增 `ed2k::infra::ClientList`，追踪 `user_hash/ip/port/last_seen`，支持 30min LRU cleanup、生成 SX2 `PeerSource`、并将已知客户端附加到 `KnownFileDB` source list（既有 UploadSession SX2 answer 继续读取 `KnownFile::sources`）。测试新增 9 个 P8-5 用例覆盖 category round-trip/Preferences 持久化/regex 归档、FriendList round-trip/find、ClientList LRU/source attachment、friend-slot 队列优先。验证：focused `Category.*:FriendList.*:ClientList.*:UploadQueue.*:Preferences.*` 15/15 pass；Windows build + ctest 400 测试 392 pass + 8 live skip，0 fail；WSL Linux root `build/linux` configure/build + ctest 400 测试 392 pass + 8 live skip，0 fail；`git diff --check` exit 0（仅 CRLF warnings）。按用户要求仅本地提交，未 push/未触发 CI。
- [x] **P8-6 Proxy + HTTPThread + collection + preview + scheduler + ChatRelay**（✅ 2026-07-05，M5/S6-S7，`2db8d62`）：新增 `ed2k::infra::ProxyConfig` 与 `Connection::connect_via_proxy`，支持 SOCKS5 no-auth CONNECT 与 HTTP CONNECT。新增 `ed2k::infra::HTTPDownload`（当时使用 HTTP GET + `Content-Length` 写文件，HTTPS 尚未支持；仅 HTTPS 状态已被后续实现取代，当前已支持经证书和主机名验证的 HTTPS，`Content-Length` 要求仍保留）、`Collection` 文本 ed2k link 列表 + 内部 `E2CL` binary round-trip、daily `SchedulerRule`/`Scheduler`、`ChatRelay` 入站消息派发并联动 FriendList friend 标志。C2C 新增 `REQUESTPREVIEW`/`PREVIEWANSWER` opcode、preview request/answer codec、aMule `OP_MESSAGE` u16 string codec；`UploadSession` 响应 preview 请求并通过 disk executor 读取前 256KiB。测试新增 13 个 focused 用例覆盖代理握手、HTTP 下载、collection、scheduler、ChatRelay、preview/chat codec、upload preview。验证：focused `Proxy.*:Connection.ConnectVia*Proxy*:HTTPDownload.*:Collection.*:Scheduler.*:ChatRelay.*:C2CMessages.Preview*:C2CMessages.Chat*:UploadSession.AnswersPreview*` 13/13 pass；Windows build + ctest 413 测试 405 pass + 8 live skip，0 fail；WSL Linux root `build/linux` configure/build + ctest 413 测试 405 pass + 8 live skip，0 fail；`git diff --check` exit 0（仅 CRLF warnings）。按用户要求仅本地提交，未 push/未触发 CI。
- [x] **P8-7 集成 + CLI + live**（✅ 2026-07-05，S8，`98f6e6c`）：统一 CLI 全局选项 `--config <preferences.dat>`/`--ipfilter <ipfilter.dat>`/`--proxy <uri>`/`--obfuscation`，并将 Preferences/IPFilter/Proxy 传入 `login`、server-backed search/sources/publish、C2C comment、Kad bootstrap/search/find/publish 与 download 启动路径；`ServerConnection` 增 `connect_and_login_via_proxy`，`login_with_rotation` 支持 SOCKS5/HTTP CONNECT 代理与共享 IPFilter。CLI 新增 `collection create/list`、`schedule add/list`、`update-serverlist <url> <dest>`。live-gated 覆盖：全局 IPFilter 在登录前阻断坏 IP、HTTP server.met 更新、SOCKS5 代理登录握手。测试新增 7 个 P8-7 focused 用例；验证：focused `CliConfig.GlobalConfigOptionSuppliesPreferencesPath:CliIPFilter.GlobalIpfilterOptionSuppliesFilterPath:CliIPFilter.GlobalIpfilterBlocksLoginBeforeConnect:CliCollection.*:CliSchedule.*:CliUpdate.*:CliProxy.*` 7/7 pass；Windows build + ctest 420 测试 412 pass + 8 live skip，0 fail；WSL Linux root `build/linux` configure/build + ctest 420 测试 412 pass + 8 live skip，0 fail；`git diff --check` exit 0（仅 CRLF warnings）。按用户要求仅本地提交，未 push/未触发 CI。
...
- [x] **P9-1 设计 spec**（✅ 已起草，2026-07-05 checklist sync）：服务器 UDP 完备/服务器列表交换/MuleInfo/AICH 共享端/压缩上传/aMule .part.met/大文件 + M1-M4 + G1-G9。
- [x] **P9-2 实现计划**（✅ 已起草，2026-07-05 checklist sync）：S1-S7 + 测试门 + 破改分析。
- [x] **P9-3 服务器 UDP 完备**（✅ 2026-07-05，M1/S1，`add4097`）：`GLOBGETSOURCES2` 请求改为 u64 size，新增 `GLOBFOUNDSOURCES2` 与 `SERVER_IDENT` UDP opcode，`decode_udp_server_ident` 复用 TCP ident 布局；`UdpServerConnection::get_sources` 等待 v2 响应并兼容旧 `GLOBFOUNDSOURCES`，pump 可派发 UDP server ident 事件。测试新增/更新 UDP opcode、u64 size、v2 found-sources concat、ident decode、v2 get_sources round-trip 与 ident event。
- [x] **P9-4 服务器列表交换 + MuleInfo**（✅ 2026-07-05，M2/S2-S3，`c24499d`）：新增 TCP `GETSERVERLIST`/`SERVERLIST` 拉取，`ServerConnection::get_server_list(timeout)` 与 `merge_server_list()` 去重合并并保留既有 metadata；CLI 新增 `ed2k-tool get-serverlist <server.met>`；新增 eMule `EMULEINFO`/`EMULEINFOANSWER` opcode、编解码与 feature 解析，`C2CConnection` 支持 MuleInfo 握手与协议感知 pump。
- [x] **P9-5 AICH 共享端 + 压缩块上传**（✅ 2026-07-05，M3/S4-S5，`6c28f32`）：AICH 共享端已由 `UploadSession` 的 `AICHFILEHASHREQ`/`AICHREQUEST` 应答与 `AICHChecker` round-trip 覆盖；本项补齐 aMule packed `OP_COMPRESSEDPART` 编码端（zlib level 1，`hash + start + compressed_size + zlib-stream`）、压缩收益判定、压缩流 ~10240B 分帧上传与下载端重组，并将 I64 C2C opcode/protocol 对齐 aMule TCP。
- [x] **P9-6 aMule-faithful .part.met + 大文件**（✅ 2026-07-05，M4/S6，`b492fd9`）：`parse/write_part_met` 支持 aMule `PARTFILE_VERSION`/`PARTFILE_VERSION_LARGEFILE`（`0xE0/0xE2`）布局：date + file hash + u16 part hash count + tag list，`FT_FILESIZE`/gap start-end named tags 与 aMule `PartFile.cpp::SavePartFile` 对齐；写出默认使用 aMule 格式并保留旧内部 `0x0E/0x0F` gap blob 读取兼容；`PartFile` 保存/恢复走 aMule met-first 信任模型，覆盖 >4GiB `Uint64` size/gap tags。
- [x] **P9-7 跨客户端续传 + live**（✅ 2026-07-06，M4/S7）：`.part` 输出使用 aMule sibling sidecar（`001.part`→`001.part.met`），`PartFile.AmulePartPathLoadsSiblingPartMet` 与 `Download.ResumesFromAmulePartMetSibling` 覆盖 mock aMule handoff；真实 aMule 2.3.3 生成 partial 后，本节点用 `ED2K_OUT=.tmp_p9_live/engine_from_amule_2p/001.part` 续传完成，Red hash `114b21c63a74b6ca922291a11177dd5c`；反向由 `LivePartMet.WritesEnginePartialFixture` 生成本节点 partial，aMule 载入后从 `LiveUpload.AcceptsLocalPeerUploadSession` 源完成下载，产物 hash 同为 `114b21c63a74b6ca922291a11177dd5c`。
...
- [x] **R1-1 设计 spec**（✅ 已起草 2026-07-04；2026-07-06 checklist sync）：现状评估（带证据）+ M1-M4 + G1-G6。
- [x] **R1-2 实现计划**（✅ 已起草 2026-07-04；2026-07-06 checklist sync）：S1-S8 + 测试门 + 破改分析。
...
- [x] **R1-4（M2/P1）结构 + 模式**：`Download::run`/`peer_worker` 拆阶段 ✅ S4（`a2fe93e`）+ 公共 API PIMPL ✅ S5（`492ef8c`）+ Observer RAII Subscription ✅ S6（`cf10db3`）。
- [x] **R1-5（M3/P2）现代 C++**：`[[nodiscard]]`/`noexcept`/`constexpr` 扫描 ✅ S7（`7bce7ad`）+ `std::hash` 强哈希 + concepts + `std::format` ✅ S8（`b8530ae`）。
...
| 2026-07-03 | P4c-3-1 完成：多源并发设计 spec 评审通过并提交（四里程碑 M1–M4，`asio::experimental::channel` 完成信号避单线程 io_context 死锁）。P4c-2-1 子代理（前次 compaction 后失效）重新派发 | 6381985 |
...
| 2026-07-03 | P4c-3-2 完成：M1 raccoon 实现计划，4 阶段 S1–S4 + 10 项陷阱清单 + 测试门 | 7c623d7 |
| 2026-07-03 | P4c-2-1 完成：AICH 两级树互操作设计 spec 评审通过（B1-B8 已核对代码，三里程碑 M1-M3）。**发现 B8 与 P4c-3 块模型冲突**——P4c-2 要求 per-part 块、P4c-3 假设 flat 块；建议 P4c-2 C3 回退先于 P4c-3 raccoon（见 §7） | 828f655 |
...
| 2026-07-03 | P4c-2-2 完成：两级 AICH 实现计划，3 阶段 S1（M1+M2 合并：`aich_checker_test` 用 `aich_hash_bytes` 当 oracle 故耦合）/S2（M3 线协议）/S3（M4 per-part 块，P4c-2→P4c-3 块模型门）+ G1-G10 陷阱 + 既有测试破改分析（`FlatMerkleTwoChunkMatchesManual` 必破、3 块巧合同根）。回填 `13fa676` | 2a0a9df |
...
| 2026-07-04 | **R1 代码质量重构 spec/plan 起草冻结**（用户反馈：封装性差/现代 C++ 不优雅/设计模式少/线程安全脆弱）。新增 spec+plan：现状评估带证据（`IPv4::value` public、`MultiSourceDownload` 8 参裸指针、`on_event` 无退订、`SharedState` 裸 bool 约定正确性、无 PIMPL/Builder/状态机、`std::hash` 弱哈希等）+ M1-M4（P0 封装+线程安全类型强化 / P1 PIMPL+拆阶段+Observer / P2 现代 C++ / P3 延后 C++23）+ S1-S8 green-deployable + G1-G6 + 测试门。纪律：行为保持 + 229 测试不回归 + aMule 字节保真不动。RELEASE-PLAN §5 加「阶段 R1」+ §7 标注正交可并行。**先做 R1-3（P0），与 P5 实现串行** | （文档，无代码提交） |
| 2026-07-06 | **R1-1/R1-2 checklist 同步**：核对 R1 代码质量重构设计 spec 与实现计划已存在并覆盖 M1-M4、S1-S8、G1-G6、测试门与 PIMPL/Observer/现代 C++ 范围；勾选 R1-1/R1-2。无代码改动，沿用上一轮 Windows/WSL 447 测试全绿证据；按用户要求仅本地提交，未 push/未触发 CI | 本地提交 |
...
| 2026-07-06 | **R1-4 S4 phase split 完成**：`Download::run` 拆为连接握手、hashset/file-status、上传启动、块分发阶段；`peer_worker` 拆为 source setup、upload、AICH 协商、pull blocks 阶段，保留既有 finish/error 语义与 aMule FILESTATUS count=0 注释。行为保持，不改协议帧。验证：Windows build `ed2k_tests` 通过；Windows focused `Download|AppDownload`：28 total，26 pass + 2 live skip，0 fail；Windows full ctest：447 total，438 pass + 9 skip，0 fail；WSL Linux `cmake --build --preset linux` + `ctest --preset linux`：447 total，438 pass + 9 skip，0 fail；`git diff --check` 仅 CRLF warning。R1-4 尚余 S5 PIMPL 与 S6 Subscription，故总项暂不勾选 | 本地提交 |
| 2026-07-06 | **R1-4 S5 公共 API PIMPL 完成**：`IoRuntime`/`Connection`/`ServerConnection`/`C2CConnection`/`PartFile`/`MultiSourceDownload` 的实现状态移入 `.cpp` `Impl`，公共对象变小型 move-only handle；保留既有 Boost.Asio coroutine API 签名，避免把 S5 扩大成 API 破坏性重写。新增 RED->GREEN `ApiPimpl.PublicAsyncTypesExposeSmallMoveOnlyHandles`，断言目标类型为小型不可拷贝 handle。验证：focused `ApiPimpl` 先失败后通过；affected `ApiPimpl|IoRuntime|Connection|ServerConnection|C2CConnection|PartFile|BlockAllocator|Download|AppDownload`：103 total，101 pass + 2 live skip，0 fail；Windows full ctest：448 total，439 pass + 9 live skip，0 fail；WSL Linux 因 `/opt/vcpkg` root-owned lock 需 `wsl -u root` 运行，`cmake --build --preset linux` + `ctest --preset linux`：448 total，439 pass + 9 live skip，0 fail；`git diff --check` 仅 CRLF warning。R1-4 尚余 S6 Subscription，故总项暂不勾选 | 本地提交 |
| 2026-07-06 | **R1-4 S6 Observer RAII Subscription 完成**：`ServerConnection::on_event` 与 `UdpServerConnection::on_event` 改为返回 move-only `Subscription`，析构自动退订；观察者存储改为 `shared_ptr` 状态 + token `weak_ptr`，保证连接先析构后 token 析构也安全；派发时复制回调列表，避免回调内退订扰乱迭代。新增 RED->GREEN `DestroyedSubscriptionStopsEvents` 覆盖 TCP/UDP 退订后不再触发。验证：S6 RED 以旧 `void on_event` API 编译失败；GREEN 后 focused `ServerConnection|UdpConnection`：31/31 pass；Windows full ctest：450 total，441 pass + 9 live skip，0 fail；WSL Linux 因 `/opt/vcpkg` root-owned lock 需 `wsl -u root` 运行，`cmake --build --preset linux` + `ctest --preset linux`：450 total，441 pass + 9 live skip，0 fail；`git diff --check` 仅 CRLF warning。**R1-4 M2/P1 全完成** | `cf10db3` |
| 2026-07-06 | **R1-5 S7 `[[nodiscard]]`/`noexcept`/`constexpr` 扫描完成**：`ByteReader` primitive/hash/blob 读取 API 增 `[[nodiscard]]`，query/primitive/hash/blob 读取增安全 `noexcept`；`MD4Hash`/`AICHHash` `from_bytes()` 与 `bytes()` 增 `constexpr noexcept`，`IPv4::from_wire()` 变为 header `constexpr noexcept`；协议常量 `PART_SIZE`/`AICH_BLOCK_SIZE`/`MAX_PACKET_SIZE` 以静态断言锁定。字符串转换 `to_hex()`/`to_base32()`/`to_dotted()` 仅加 `[[nodiscard]]`，不标 `noexcept`，因为返回 `std::string` 可能分配。新增 RED->GREEN 编译期契约断言；GREEN 后修正 AICH 解析中有意跳过字段的 `(void)` casts，保持 GCC `[[nodiscard]]` warning-clean。验证：RED build 因缺少 `noexcept`/`constexpr` static_assert 失败；focused `ByteIO|MD4Hash|AICHHash|IPv4|C2CMessages|C2CConnection`：86/86 pass；Windows full ctest：450 total，441 pass + 9 live skip，0 fail；WSL Linux `wsl -u root` build + ctest：450 total，441 pass + 9 live skip，0 fail，且无新增 nodiscard warnings；`git diff --check` 仅 CRLF warning | 本地提交 |
| 2026-07-06 | **R1-5 S8 强哈希 + concepts + `std::format` 完成**：`std::hash<MD4Hash>` 从弱 polynomial `s=s*131+byte` 改为 64-bit FNV-1a 风格逐字节混合，新增 crafted polynomial collision 回归；`codec::ByteRange` concept 约束 `std::byte` 连续 range，`ByteReader`/`ByteWriter::blob` 增 concept overload，并静态拒绝 `std::vector<uint8_t>`；`MD4Hash::to_hex()` 与 `IPv4::to_dotted()` 改用 C++20 `std::format`/`format_to`（GCC 13.3/MSVC gate 验证）。验证：S8 RED build 因 `ByteRange` concept 缺失失败；focused `ByteIO|MD4Hash|AICHHash|IPv4|Link|KnownFile|ClientCredits|FriendList|ClientList`：28/28 pass；Windows full ctest：452 total，443 pass + 9 live skip，0 fail；WSL Linux `wsl -u root` build + ctest：452 total，443 pass + 9 live skip，0 fail；`git diff --check` 仅 CRLF warning。**R1-5 M3/P2 全完成** | `b8530ae` |
| 2026-07-06 | **v2.2.0 发布完成**：运行时版本、`project(VERSION)`、`vcpkg.json`、README、CHANGELOG 与 RELEASE-PLAN 当前版本同步到 `2.2.0`；新增 `Smoke.VersionMatchesRelease` 锁定 `ed2k::version()` 与发布号一致。验证：focused `Smoke.*` 2/2 pass；Windows build `ed2k_tests`/`ed2k-tool` + full ctest：453 total，444 pass + 9 live skip，0 fail；WSL Linux `wsl -u root` build + `ctest --preset linux`：453 total，444 pass + 9 live skip，0 fail；`git diff --check` 仅 CRLF warning；CI run `28765725134`：ubuntu-latest + windows-2022 均通过。发布：annotated tag `v2.2.0` 指向 release commit `5ca81d2` 并已推送，GitHub Release（非 draft、非 pre-release）发布：https://github.com/cool2528/ed2k-engine-cpp/releases/tag/v2.2.0；release 记录随后追加到 `main` 并由 CI run `28766304122` 双平台验证通过 | `5ca81d2` + tag `v2.2.0` |
...
| 2026-07-05 | **P5-8 测试 + live 完成**：补齐 eMule HELLO SX2/multipacket 能力与 aMule 兼容 `OP_MULTIPACKET` SX2 请求；`UploadSession` 可处理 multipacket 文件名/文件状态/AICH/SX2 子操作；`REQFILENAMEANSWER` 解码兼容 aMule `u16` 字符串。`LiveUpload.SourceExchange2WithLocalPeer` 构造 holder+requester 两个临时身份，holder 入 aMule 上传列表后刷新 filename context，requester SX2 断言返回 holder 源。验证：WSL aMule 2.3.3 live 绿；Windows `cmake --build build/default --config Debug --target ed2k_tests ed2k-tool -- /m` + `ctest -C Debug -j8 --output-on-failure`：288 测试 281 pass + 7 live skip，0 fail。按用户要求仅本地提交，未 push/未触发 GitHub Actions | `72793b3` |
| 2026-07-05 | **P6-1/P6-2 checklist 同步**：核对 P6 KAD 设计 spec 与实现计划已存在并覆盖 KadID/路由表/Kad2 UDP/搜索/发布/buddies/live 计划，勾选 RELEASE-PLAN 的 P6-1/P6-2；下一项为 P6-3 KadID + 路由表 + nodes.dat。文档同步，无代码改动，沿用上一轮 `ctest -C Debug -j8 --output-on-failure`：288 测试 281 pass + 7 live skip，0 fail | `f0871a7` |
| 2026-07-05 | **P6-3 KadID + 路由表 + nodes.dat 完成**（`e329bfa`）：新增 `ed2k::kad::KadID`（hex/bit/XOR distance/MD4(userhash+nonce) 生成）、`Contact`/`KBucket`/`RoutingZone`/`RoutingTable`（k=10、LRU update、本机 ID 范围内分裂、稀疏 sibling 合并、最近 k 查找）和 compact `nodes.dat` round-trip。新增 9 个 KAD S1 单测；附带 `09c8105` 将 `ServerConnection.PublishFilesSendsOfferFilesFrame` mock 捕获改为 ACK 同步，修复 Linux 单线程调度竞态。验证：Windows `cmake --build build/default --config Debug --target ed2k_tests ed2k-tool -- /m` + `ctest -C Debug -j8 --output-on-failure`：297 测试 290 pass + 7 live skip，0 fail；WSL Linux（`PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin` 避免 vcpkg 扫 Windows PATH）`cmake --build --preset linux` + `ctest --preset linux`：297 测试 290 pass + 7 live skip，0 fail | `e329bfa` |
| 2026-07-05 | **P6-4 Kad2 UDP + Hello + bootstrap 完成**（`4666a1c`）：新增 `ed2k::kad::messages`（Kad2 constants + HELLO/REQ/RES codecs）与 `KadNetwork`（one-shot serve、HELLO round-trip、closest-node REQ/RES、seed bootstrap），并给 `RoutingTable` 增 `find`。字节级对照 aMule 修正 P6 草稿旧值：Kad protocol `0xE4`，HELLO_REQ/RES `0x11/0x19`，REQ/RES `0x21/0x29`，Hello payload 为 `KadID(16)+tcp_port(2)+version(1)+tagCount(1)+tags`；`REQ` receiver-check KadID 锁定目标节点，wrong receiver 不响应。新增 6 个测试覆盖 payload bytes、malformed rejection、routing update、closest contacts、防伪造 receiver check、bootstrap learn contacts。边界记录：aMule UDP sender/receiver verify key 来自 encrypted datagram 层，非 clear Hello 字段，完整 crypto verify 归 P7 混淆。验证：Windows `cmake --build build/default --config Debug --target ed2k_tests ed2k-tool -- /m` + `ctest -C Debug -j8 --output-on-failure`：303 测试 296 pass + 7 live skip，0 fail；WSL Linux（sanitized PATH + `VCPKG_ROOT=/opt/vcpkg`）`cmake --build --preset linux` + `ctest --preset linux`：303 测试 296 pass + 7 live skip，0 fail | `4666a1c` |
| 2026-07-05 | **P6-5 搜索/取源/发布完成**（`a2e3ae6`）：新增 aMule-faithful Kad2 `SEARCH_KEY/SOURCE/NOTES_REQ`、`SEARCH_RES`、`PUBLISH_KEY/SOURCE/NOTES_REQ`、`PUBLISH_RES` 编解码；新增 `KadIndexed` TTL 索引（关键词、源、notes 三类）和 `KadNetwork` 发布/搜索协程，支持 peer 聚合、answer_id 去重、file size 过滤，以及空结果后经 closest-node REQ bounded iterative 扩展搜索。新增 11 个 P6-5 测试覆盖 wire layout、TTL/size/dedupe、发布后搜索、closest-node 发现 indexer、跨 peer 源去重与 notes publish/search。验证：Windows `cmake --build build/default --config Debug --target ed2k_tests ed2k-tool -- /m` + `ctest --test-dir build/default -C Debug -j8 --output-on-failure`：314 测试 307 pass + 7 live skip，0 fail；WSL Linux（sanitized PATH + `VCPKG_ROOT=/opt/vcpkg`）`cmake --build --preset linux` + `ctest --preset linux`：314 测试 307 pass + 7 live skip，0 fail | `a2e3ae6` |
| 2026-07-05 | **P6-6 Firewalled + buddies 完成**（`71243be`）：按 aMule 当前源码补齐 Kad firewalled/buddy/callback 帧：`KADEMLIA_FIREWALLED_REQ/RES/ACK`、`KADEMLIA_FIREWALLED2_REQ`、`KADEMLIA_FINDBUDDY_REQ/RES`、`KADEMLIA_CALLBACK_REQ`、`KADEMLIA2_FIREWALLUDP`。`KadNetwork` 新增 UDP firewall state、TCP firewalled check、observed external IP、buddy active/inactive state、find-buddy round-trip 与 Kad callback event。新增 8 个 P6-6 测试覆盖 wire layout、malformed reject、firewall 检测、buddy 建立/失活与 callback。验证：Windows `cmake --build build/default --config Debug --target ed2k_tests ed2k-tool -- /m` + `ctest --test-dir build/default -C Debug -j8 --output-on-failure`：322 测试 315 pass + 7 live skip，0 fail；WSL Linux（sanitized PATH + `VCPKG_ROOT=/opt/vcpkg`）`cmake --build --preset linux` + `ctest --preset linux`：322 测试 315 pass + 7 live skip，0 fail | `71243be` |
| 2026-07-05 | **P6-7 集成 + CLI + live 完成**（`6deabe6`）：`MultiSourceDownload` 可从注入的 `KadNetwork` 追加 Kad 直连源；CLI 新增 `kad-bootstrap`/`kad-search`/`kad-find-sources`/`kad-publish`；README/USAGE/API 同步。live 调试按 aMule 修正 UInt128 wire/crypto 字节序、Kad packed UDP inflate、public `nodes.dat` 变体、closest-node + `SEARCHTOLERANCE` 搜索策略，以及 tag 类型表（`BLOB=0x07`、`BSOB=0x0A`、`UINT64=0x0B`、`STR1..STR22`），公网 `kad-bootstrap` loaded=191/routing=49，`kad-search ubuntu` 返回 470 results。验证：Windows `ctest --test-dir build/default -C Debug -j8 --output-on-failure`：352 测试 344 pass + 8 live skip，0 fail；WSL Linux `cmake --build --preset linux` + `ctest --preset linux`：352 测试 344 pass + 8 live skip，0 fail。按用户要求仅本地提交，未 push | `6deabe6` |
| 2026-07-05 | **P7-1/P7-2 checklist 同步**：核对 P7 协议混淆设计 spec 与实现计划已存在并覆盖 MD5/RC4、TCP/UDP/服务器混淆、协商 fallback、live 计划、G1-G8 陷阱与 S1-S6 测试门；下一项为 P7-3 MD5/RC4 自带实现。文档同步，无代码改动，沿用上一轮 Windows/WSL 352 测试全绿证据 | 本地提交 |
| 2026-07-05 | **P7-3 MD5/RC4 自带实现完成**（`bd74243`）：新增共享 `ed2k::crypto::RC4`（KSA/PRGA、buffer/byte 处理、discard、跨调用 keystream 续接）并接入 `ed2k_core`；`tests/rc4_test.cpp` 覆盖 RC4 已知向量、续接、逐字节等价、discard；Kad UDP obfuscation 改用共享 RC4，移除私有重复实现，既有 aMule 对照 Kad UDP crypto 测试保持绿。验证：Windows `cmake --build build/default --config Debug --target ed2k_tests ed2k-tool -- /m` + `ctest --test-dir build/default -C Debug -j8 --output-on-failure`：356 测试 348 pass + 8 live skip，0 fail；WSL Linux（sanitized PATH + `VCPKG_ROOT=/opt/vcpkg`）`cmake --build --preset linux` + `ctest --preset linux`：356 测试 348 pass + 8 live skip，0 fail；`git diff --check` exit 0。按用户要求仅本地提交，未 push/未触发 CI | `bd74243` |
| 2026-07-05 | **P7-4 TCP 混淆握手 + 帧加密完成**（`84ad587`）：新增 `ed2k::net::EncryptedStreamSocket`，实现 aMule basic peer obfuscation clear marker/random-key 前缀、`MD5(user_hash || role_magic || random_key_part)` RC4 key 派生、1024 字节 discard、sync/method/padding 握手，以及握手后 eD2k frame 透明加解密；测试新增 6 个 `EncryptedStreamSocket.*`，覆盖 aMule 确定性握手向量、plain marker/wrong magic 拒绝、加密 frame round-trip、双向多帧 RC4 续接。验证：focused `EncryptedStreamSocket.*:Connection.*:RC4.*:KadUdpCrypto.*` 21/21 pass；Windows `cmake --build build/default --config Debug --target ed2k_tests ed2k-tool -- /m` + `ctest --test-dir build/default -C Debug -j8 --output-on-failure`：362 测试 354 pass + 8 live skip，0 fail；WSL Linux（sanitized PATH + `VCPKG_ROOT=/opt/vcpkg`）`cmake --build --preset linux` + `ctest --preset linux`：362 测试 354 pass + 8 live skip，0 fail；`git diff --check` exit 0。按用户要求仅本地提交，未 push/未触发 CI | `84ad587` |
| 2026-07-05 | **P7-5 UDP + 服务器混淆 + 协商 fallback 完成**（`0440e19`）：新增 `ed2k::net::udp_obfuscation` aMule server UDP codec（8-byte header、base key + direction magic + random key part MD5、RC4、sync `0x13EF24D5`，client/server deterministic vectors）；`server.met` obfuscation tags 持久化；`UdpServerConnection` 使用 obfuscated UDP port 探测并在 timeout 后 plain fallback；HELLO misc-options2 obfuscation support/request/require bits；`ServerTarget` 保留服务器 obfuscation metadata。验证：Windows build + ctest 372 测试 364 pass + 8 live skip，0 fail；WSL Linux root `build/linux` build + ctest 372 测试 364 pass + 8 live skip，0 fail；`git diff --check` exit 0（仅 CRLF warnings）。本切片未声明真实 live obfuscation server/peer 通过；按用户要求仅本地提交，未 push/未触发 CI | `0440e19` |
| 2026-07-05 | **P8-1/P8-2 checklist 同步**：核对 P8 客户端基础设施设计 spec 与实现计划已存在并覆盖 IPFilter、Preferences、Statistics、Category、FriendList、ClientList、Proxy、HTTPThread、collection、preview、scheduler、ChatRelay、M1-M5、S1-S8、G1-G10 与 live 计划；下一项为 P8-3 IPFilter。文档同步，无代码改动，沿用上一轮 Windows/WSL 372 测试全绿证据；按用户要求仅本地提交，未 push/未触发 CI | 本地提交 |
| 2026-07-05 | **P8-3 IPFilter 完成**（`a4cf0e3`）：新增 `ed2k::infra::IPFilter`（range/CIDR parser、level threshold、sorted lookup + prefix max-end overlap guard）和 `errc::ip_filtered`；接入 `net::Connection`、server/C2C wrappers、download peer/Kad source、Kad UDP outbound、upload accepted remote pre-HELLO；CLI 新增 `ed2k-tool ipfilter`。测试新增 10 个 focused 用例覆盖 parser、threshold、overlap、malformed、CLI block-check 与 server/C2C/Kad/upload boundary reject。验证：focused 10/10 pass；Windows build + ctest 382 测试 374 pass + 8 live skip，0 fail；WSL Linux root `build/linux` configure/build + ctest 382 测试 374 pass + 8 live skip，0 fail；`git diff --check` exit 0（仅 CRLF warnings）。按用户要求仅本地提交，未 push/未触发 CI | `a4cf0e3` |
| 2026-07-05 | **P8-4 Preferences + Statistics 完成**（`291c87e`）：新增 `ed2k::infra::Preferences`（引擎配置内部 `E2PF` taglist 格式 round-trip）和 `ed2k::infra::Statistics`（session + cumulative 计数、cumulative-only `E2TA` 持久化、disk executor 异步 flush）；CLI 新增 `ed2k-tool config` / `ed2k-tool stats`。测试新增 9 个 focused 用例覆盖默认值、round-trip、bad magic、计数、持久化、异步 flush 与 CLI。验证：focused 9/9 pass；Windows build + ctest 391 测试 383 pass + 8 live skip，0 fail；WSL Linux root `build/linux` configure/build + ctest 391 测试 383 pass + 8 live skip，0 fail；`git diff --check` exit 0（仅 CRLF warnings）。按用户要求仅本地提交，未 push/未触发 CI | `291c87e` |
| 2026-07-05 | **P8-5 Category + FriendList + ClientList 完成**（`2e37a2c`）：新增 `ed2k::infra::Category`/`CategoryList`（`E2CT` round-trip + Preferences categories blob + regex 归档路径）、`FriendList`（`E2FR` round-trip + user hash/KadID/name/friend-slot）和 `ClientList`（LRU cleanup + SX2 `PeerSource` 生成 + KnownFileDB source attachment）；`UploadQueue` 支持注入 FriendList，使 friend-slot peer 排队优先。测试新增 9 个 P8-5 用例；验证：focused 15/15 pass；Windows build + ctest 400 测试 392 pass + 8 live skip，0 fail；WSL Linux root `build/linux` configure/build + ctest 400 测试 392 pass + 8 live skip，0 fail；`git diff --check` exit 0（仅 CRLF warnings）。按用户要求仅本地提交，未 push/未触发 CI | `2e37a2c` |
| 2026-07-05 | **P8-6 Proxy + HTTPThread + collection + preview + scheduler + ChatRelay 完成**（`2db8d62`）：新增 SOCKS5/HTTP CONNECT 代理连接、HTTP GET 下载、collection 文本 ed2k link + 内部 binary round-trip、daily scheduler、ChatRelay 入站派发 + FriendList 标志、preview/chat C2C codec，以及 `UploadSession` preview 前 256KiB 响应。测试新增 13 个 focused 用例；验证：focused 13/13 pass；Windows build + ctest 413 测试 405 pass + 8 live skip，0 fail；WSL Linux root `build/linux` configure/build + ctest 413 测试 405 pass + 8 live skip，0 fail；`git diff --check` exit 0（仅 CRLF warnings）。按用户要求仅本地提交，未 push/未触发 CI | `2db8d62` |
| 2026-07-05 | **P8-7 集成 + CLI + live-gated 完成**（`98f6e6c`）：统一 CLI 全局 `--config`/`--ipfilter`/`--proxy`/`--obfuscation`，新增 `collection create/list`、`schedule add/list`、`update-serverlist`；`ServerConnection`/`login_with_rotation` 支持代理登录与共享 IPFilter，CLI server/Kad/download/comment 启动路径加载 Preferences/IPFilter/Proxy。测试新增 7 个 focused 用例覆盖全局 config/ipfilter、登录前 IPFilter 阻断、collection/schedule、HTTP update、SOCKS5 代理登录。验证：focused 7/7 pass；Windows build + ctest 420 测试 412 pass + 8 live skip，0 fail；WSL Linux root `build/linux` configure/build + ctest 420 测试 412 pass + 8 live skip，0 fail；`git diff --check` exit 0（仅 CRLF warnings）。按用户要求仅本地提交，未 push/未触发 CI | `98f6e6c` |
| 2026-07-05 | **P9-1/P9-2 checklist 同步**：核对 P9 eD2k 协议完备性设计 spec 与实现计划已存在并覆盖服务器 UDP 完备、服务器列表交换、MuleInfo、AICH 共享端、压缩块上传、aMule-faithful `.part.met`、大文件边界、S1-S7、G1-G9 与 live 计划；下一项为 P9-3 服务器 UDP 完备。文档同步，无代码改动，沿用 P8-7 Windows/WSL 420 测试全绿证据；按用户要求仅本地提交，未 push/未触发 CI | 本地提交 |
| 2026-07-05 | **P9-3 服务器 UDP 完备完成**（`add4097`）：补齐 P9 M1/S1 服务器 UDP v2 源查询与 ident surface：`GLOBGETSOURCES2` payload 写 u64 size，新增 `GLOBFOUNDSOURCES2`/`SERVER_IDENT` opcode，`decode_udp_server_ident` 复用 TCP ident 布局；`UdpServerConnection::get_sources` 等待 v2 响应并兼容旧 `GLOBFOUNDSOURCES`，pump 可派发 UDP server ident event。测试新增/更新 6 个 P9-3 用例；验证：focused P9-3 8/8 pass，`UdpMessages.*:UdpConnection.*` 37/37 pass；Windows build + ctest 426 测试 418 pass + 8 live skip，0 fail；WSL Linux root `build/linux` configure/build + ctest 426 测试 418 pass + 8 live skip，0 fail；`git diff --check` exit 0（仅 CRLF warnings）。按用户要求仅本地提交，未 push/未触发 CI | `add4097` |
| 2026-07-05 | **P9-4 服务器列表交换 + MuleInfo 完成**（`c24499d`）：补齐 P9 M2/S2-S3：`ServerConnection::get_server_list(timeout)` 发送 TCP `GETSERVERLIST` 并接收 `SERVERLIST`；`merge_server_list()` 合并远端列表并按 endpoint 去重、保留既有 metadata；CLI 新增 `ed2k-tool get-serverlist <server.met>`；新增 eMule `EMULEINFO`/`EMULEINFOANSWER` opcode、MuleInfo codec、client feature 解析，以及 `C2CConnection` MuleInfo 握手与协议感知 pump。测试新增/更新 P9-4 server/C2C/CLI/server.met 用例，并修正 AICH mock fixture 使用 `OP_EMULEPROT` 响应 AICH。验证：focused P9-4 8/8 pass；受影响套件 `ServerMessages.*:ServerConnection.*:ServerMet.*:C2CMessages.*:C2CConnection.*:CliServerList.*` 115/115 pass；Windows build `ed2k_tests`/`ed2k-tool` + ctest 433 测试 425 pass + 8 live skip，0 fail；WSL Linux root configure/build/ctest 433 测试 425 pass + 8 live skip，0 fail；`git diff --check` exit 0（仅 CRLF warnings）。按用户要求仅本地提交，未 push/未触发 CI | `c24499d` |
| 2026-07-05 | **P9-5 AICH 共享端 + 压缩块上传完成**（`6c28f32`）：确认既有 AICH 共享端 `AICHFILEHASHREQ`/`AICHREQUEST` 应答与 `AICHChecker` round-trip 已覆盖 S4；补齐 S5 压缩上传：新增 `encode_compressed_part`/I64 编码，使用 aMule packed header `hash + start + compressed_size + zlib-stream` 与 zlib level 1；`UploadSession` 仅在压缩后更小时发送 eMule `OP_COMPRESSEDPART`，按 ~10240B 切分压缩流，否则回退 `SENDINGPART`；`C2CConnection` 可重组 split compressed stream；I64 opcode 修正为 aMule TCP `0xA1/0xA2/0xA3` 且 `REQUESTPARTS_I64` 使用 eMule protocol；Hello compression bit 编解码补齐。测试新增/更新 packed header、I64 opcode、compression bit、split compressed reassembly、压缩收益判定与上传分帧用例；验证：新增 split compressed RED 后 GREEN；focused `C2CMessages|C2CConnection|UploadSession` 99 测试 98 pass + 1 live skip，0 fail；受影响套件 `C2CMessages|C2CConnection|UploadSession|Download|AppDownload|LiveUpload` 126 测试 122 pass + 4 live skip，0 fail；Windows build `ed2k_tests`/`ed2k-tool` + ctest 441 测试 433 pass + 8 live skip，0 fail；WSL Linux mounted repo build/ctest 441 测试 433 pass + 8 live skip，0 fail；`git diff --check` exit 0（仅 CRLF warnings）。按用户要求仅本地提交，未 push/未触发 CI | `6c28f32` |
| 2026-07-05 | **P9-6 aMule-faithful `.part.met` + 大文件完成**（`b492fd9`）：按 aMule `PartFile.cpp::SavePartFile`/`LoadPartFile` 对照补齐 `.part.met` 格式：解析 `0xE0/0xE1/0xE2`，写出 `0xE0` 或 large `0xE2`，布局为 date + file hash + u16 part hash count + part hashes + tag list；`FT_FILESIZE` 用 `Uint32/Uint64`，gap named tags 使用 `FT_GAPSTART/FT_GAPEND + index` 且内部 half-open gap 与 aMule inclusive gap `end + 1` 对齐；保留旧内部 `0x0E/0x0F` gap blob 读取兼容。`PartFile` 保存改用 aMule 格式并继续 met-first 信任恢复，损坏/陈旧 met 回退 rehash；>4GiB size/gap tags 使用 `Uint64`。本轮同时修正 `CMakePresets.json` Linux preset 独立使用 `build/linux`，避免复用 Windows `build/default` Visual Studio cache，使文档指定的 WSL `cmake --build --preset linux && ctest --preset linux` 可直接运行。验证：focused `PartMet|PartFile` 18/18 pass；Windows build `ed2k_tests`/`ed2k-tool` + ctest 444 测试 436 pass + 8 live skip，0 fail；WSL Linux `cmake --build --preset linux && ctest --preset linux`：444 测试 436 pass + 8 live skip，0 fail。按用户要求仅本地提交，未 push/未触发 CI | `b492fd9` |
| 2026-07-06 | **P9-7 跨客户端续传进展（mock handoff）**：定位 aMule handoff 缺口为 sidecar 命名：既有 `PartFile` 对任意输出使用 `<out>.part.met`，导致 `001.part` 查找/写出 `001.part.part.met` 而非 aMule sibling `001.part.met`。新增 RED `PartFile.AmulePartPathLoadsSiblingPartMet` 后修正 `.part` 输出的 met 路径规则，并新增 `Download.ResumesFromAmulePartMetSibling` 覆盖 mock aMule `.part/.part.met` handoff 到 `MultiSourceDownload` 的续传流程，断言不生成非 aMule sidecar。文档同步 `USAGE.md`/`API.md`。验证：focused `PartFile.AmulePartPathLoadsSiblingPartMet|Download.ResumesFromAmulePartMetSibling` 2/2 pass；真实 aMule 双向 live 未完成，P9-7 暂不勾选 | 本地提交 |
| 2026-07-06 | **P9-7 跨客户端续传 + live 完成**：真实双向 handoff 通过。① aMule -> 本节点：aMule 2.3.3 生成 `.tmp_p9_live/engine_from_amule_2p/001.part` + sibling `.part.met`（测试文件 `p9-handoff-2p.bin`，size `19456000`，Red hash `114b21c63a74b6ca922291a11177dd5c`），本节点 `LiveDownload.LocalPeerCompletes` 使用 `ED2K_OUT=/mnt/d/workspace/ed2k-engine-cpp/.tmp_p9_live/engine_from_amule_2p/001.part` 续传完成，`ed2k-tool hash --red` 校验同 hash。② 本节点 -> aMule：`LivePartMet.WritesEnginePartialFixture` 写出 `.tmp_p9_live/amule_from_engine_2p/001.part(.met)`，aMule 载入为 50% partial，经 source link `sources,172.22.37.147:24668` 从 `LiveUpload.AcceptsLocalPeerUploadSession` 下载完成；上传 harness 通过，aMule Incoming 产物 Red hash 同为 `114b21c63a74b6ca922291a11177dd5c`。P9-7 勾选，P9 全部完成。验证：Windows build `ed2k_tests`/`ed2k-tool` + ctest 447 测试 438 pass + 9 skip，0 fail；WSL Linux `cmake --build --preset linux` + `ctest --preset linux`：447 测试 438 pass + 9 skip，0 fail。按用户要求仅本地提交，未 push/未触发 CI | 本地提交 |
| 2026-07-15 | **aMule optional 上传 live gate 首次通过**：定位并修复两个根因。① harness 源地址取 `hostname -I` 首字段——2026-07-12 运行解析为 Windows vEthernet 网关 `172.28.192.1`（实验证实 WSL 内连该地址超时、连 eth0 地址即通），改为 `ip -4 -o addr show dev eth0 scope global` 确定性推导；② 逐行对照 aMule 2.3.3 `PartFile.cpp::Process`：`DS_NONE` 等分支的 `AskForDownload` 均被 `theApp->IsConnected()` 门控，完全隔离（无服务器/无 Kad）的 aMule 会登记注入源（`Total sources: 1`）但永不发起连接。修复：新增 `LiveServerStub.ServesAmuleLoginUntilTerminated` harness 工具测试（本地最小 eD2k 服务器，`LOGINREQUEST`→`IDCHANGE` HighID，其余帧静默忽略，SIGTERM 结束）；harness 先起 stub、经 `amulecmd Add ed2k://\|server\|...` + `Connect ip:port` 让 aMule 入网（轮询 `eD2k: Connected`），再注入文件源并轮询 `Total sources > 0` 快速失败；conf 增 `ConnectToED2K=1`（`Autoconnect=0` 保持不自连）与 `Ed2kServersUrl=disabled`（封堵 aMule 空列表启动时自动公网拉取 server.met，`://` 缺失走 Invalid URL 短路）；`amulecmd` 统一 `LC_ALL=C`；成功路径显式 `exit 0`（清除 `-AllowFailure` 探测残留的 `$LASTEXITCODE`）。结构单测 `run-amule-obfuscation.tests.ps1` 断言 stub→daemon→listener→connect→Add→源验证顺序并禁用 `hostname -I`。证据：optional 模式两次全绿——4 混淆断言 + `LiveUpload.AcceptsLocalPeerUploadSession` 54s 内 aMule 从注入源完整下载 19,456,000 字节落盘 incoming（第二次运行确认零公网访问，amuled 日志"无效 URL"）；required 模式复验 5/5 绿、exit 0；Windows ctest 535 测试 519 pass + 16 live skip 0 fail；WSL Linux ctest 524 测试 508 pass + 16 live skip 0 fail | 本地提交 |
| 2026-07-19 | **v2.3.0 发布（补记）**：Session facade（`ed2k::session::Session`，任务注册/编排/调度、服务器连接管理+`server.met`持久化、服务器端 `search`、可选 Kad 参与、分享/上传）并入 `ed2k/ed2k.hpp` 总头文件，`CMakeLists.txt`/`vcpkg.json`/`README.md`/`CHANGELOG.md` 版本同步至 `2.2.0`→`2.3.0`；`docs/API.md`/`docs/API.zh-CN.md` 新增完整 Session 契约（线程契约、状态机、Phase 0 已知限制）。**本文件（RELEASE-PLAN.md）当时未同步更新**（当前版本字段停留在 `2.2.0`），本行为补记，避免与 CHANGELOG/README 的版本记录脱节。验证（发布时记录）：Windows Debug ctest 557 total，541 pass + 16 live-gated skip，0 fail；install + 独立 consumer 包 smoke 确认 `session/session.hpp`、`ed2kConfig.cmake` 存在 | `f6993d3` |
| 2026-07-20 | **Phase 2a（GUI search API 打底）+ v2.4.0 发布完成**：五项改动——① `server::SearchResultItem` 从搜索结果 tag 提取 `sources`/`complete_sources`（`3521fda`）；② `ServerConnection` 新增 `OP_QUERYMORERESULTS`(0x21) 连接层翻页（`45456b2`）；③ `Session::search_more()` 门面，取上一次 `search` 的下一批结果，须与 `search` 同连接串行调用，未连接→`connect_failed`，空列表=服务器无更多结果（`b2f9b7b`）；④ `ServerInfo` 补 `users`/`files`/`max_users`（静态值来自 `server.met`，已连接行的 `users`/`files` 用连接期实时值覆盖，`max_users` 仅静态值），并将原生产态 `ED2K_EXPORT` 测试后门 `test_only_set_server_static_stats` 移除、改用真实 `server.met` round-trip 锁定（`f1078ce`/`7d4f4aa`/`e6c6c57`）；⑤ `Session::kad_search(keyword)` 门面，内部构造临时 ephemeral `KadNetwork` 实例查询（避免与常驻 `kad_run` 协程争抢主 Kad socket 单读者），Kad 未启用或路由表为空→`connect_failed`，超时→`errc::timed_out`（非空结果）（`67a2ab9`）。`docs/API.md`/`docs/API.zh-CN.md` 同步 `ServerInfo` 新字段与 `search_more`/`kad_search` 两个新门面签名+语义。版本号五处（`CMakeLists.txt`/`vcpkg.json`/`README.md`/`CHANGELOG.md`/本文件「当前版本」）`2.3.0`→`2.4.0`，`tests/smoke_test.cpp` 的 `Smoke.VersionMatchesRelease`/`CompileTimeVersionConstants` 同步更新。验证：Windows `cmake --build build/default --config Release --parallel` 通过；`ed2k_tests.exe --gtest_filter="Smoke.*"` 3/3 pass（含 `Smoke.VersionMatchesRelease` 锁定 `2.4.0`）；`ctest --test-dir build/default -C Release`：565 测试 549 pass + 16 live skip，0 fail。仅本地提交，未 tag、未 push、未触发 CI | 本地提交 |
...
ed2k-tool get-serverlist <server.met>                   # 从服务器拉取并合并 server.met（P9-4）
...
ed2k-tool publish <dir> [--server:server.met] [--ip:x.x.x.x] [--port:n]  # 发布共享（P5）
ed2k-tool comment <ed2k-link> --rating:n --comment:text [--peer:ip:port] # 评论/评分（P5）
ed2k-tool ipfilter <ipfilter.dat> [--block-check:ip] [--level:n]         # IPFilter 检查（P8-3）
ed2k-tool config <preferences.dat> [--set:key=value]                     # 配置读写（P8-4）
ed2k-tool stats <statistics.dat>                                          # 统计打印（P8-4）
ed2k-tool collection create <collection> <ed2k-link>...  # collection 写入（P8-7）
ed2k-tool collection list <collection>                   # collection 列表（P8-7）
ed2k-tool schedule add <rules.txt> <rule>                # scheduler 规则写入（P8-7）
ed2k-tool schedule list <rules.txt>                      # scheduler 规则列表（P8-7）
ed2k-tool update-serverlist <url> <dest>                 # HTTP 更新 server.met（P8-7）
ed2k-tool kad-bootstrap <nodes.dat>                     # Kad bootstrap（P6-7）
ed2k-tool kad-search <nodes.dat> <keyword>              # Kad 关键词搜索（P6-7）
ed2k-tool kad-find-sources <nodes.dat> <ed2k-link>      # Kad 取源（P6-7）
ed2k-tool kad-publish <nodes.dat> <dir> [--port:n]      # Kad 发布（P6-7）
...
- 各阶段设计 spec / 实现 plan：内部文档（不随仓库发布）
