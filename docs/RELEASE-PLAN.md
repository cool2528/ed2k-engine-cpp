...
| **v1.1** | P5 上传/分享/信用 | 共享索引 + 服务器发布 + 上传会话 + 上传队列+限速 + 信用系统 + SourceExchange v2 + 评论/评分（spec/plan 已起草） | ✅ P5 完成：M1/S1 + M2/S2-S4 上传/发布 + M3/S5-S7 上传队列/限速/信用 + M4/S8 SourceExchange/评论 + P5-8 aMule live 绿（[spec](superpowers/specs/2026-07-04-ed2k-p5-upload-sharing-credits-design.md)/[plan](superpowers/plans/2026-07-04-ed2k-p5-upload-sharing-credits-impl-plan.md)） |
| **v1.2** | P6 KAD | Kad 路由表 + bootstrap + Kad2 UDP + 搜索/取源/发布 + buddies + notes（spec/plan 已起草） | ✅ P6 完成：P6-1..P6-7 ✅，公网 Kad bootstrap/search live 绿（[spec](superpowers/specs/2026-07-04-ed2k-p6-kad-design.md)/[plan](superpowers/plans/2026-07-04-ed2k-p6-kad-impl-plan.md)） |
| **v2.0** | P7 协议混淆 | MD5/RC4 + TCP/UDP/服务器混淆 + 握手协商 + fallback（spec/plan 已起草） | ✅ P7 完成：P7-1..P7-5 ✅（[spec](superpowers/specs/2026-07-04-ed2k-p7-obfuscation-design.md)/[plan](superpowers/plans/2026-07-04-ed2k-p7-obfuscation-impl-plan.md)） |
| **v2.1** | P8 客户端基础设施 | IPFilter + Preferences + Statistics + Category + FriendList + ClientList + Proxy + HTTPThread + .emulecollection + preview + scheduler + **ChatRelay（OP_MESSAGE 中继）**（spec/plan 已起草） | ✅ P8 完成：P8-1..P8-7 ✅（[spec](superpowers/specs/2026-07-04-ed2k-p8-client-infra-design.md)/[plan](superpowers/plans/2026-07-04-ed2k-p8-client-infra-impl-plan.md)） |
| **v2.2** | P9 eD2k 协议完备性 | 服务器 UDP 完备 + 服务器列表交换 + MuleInfo + AICH 共享端 + 压缩块上传 + aMule-faithful .part.met（跨客户端续传）+ 大文件边界（spec/plan 已起草） | ✅ P9 完成：P9-1..P9-7 ✅，跨客户端 aMule ↔ 本节点 live 绿（[spec](superpowers/specs/2026-07-04-ed2k-p9-protocol-completeness-design.md)/[plan](superpowers/plans/2026-07-04-ed2k-p9-protocol-completeness-impl-plan.md)） |
...
| 2026-07-06 | **R1-1/R1-2 checklist 同步**：核对 [R1 代码质量重构设计 spec](superpowers/specs/2026-07-04-ed2k-r1-code-quality-refactor-design.md) 与 [R1 代码质量重构实现计划](superpowers/plans/2026-07-04-ed2k-r1-code-quality-refactor-impl-plan.md) 已存在并覆盖 M1-M4、S1-S8、G1-G6、测试门与 PIMPL/Observer/现代 C++ 范围；勾选 R1-1/R1-2。无代码改动，沿用上一轮 Windows/WSL 447 测试全绿证据；按用户要求仅本地提交，未 push/未触发 CI | 本地提交 |
...
| 2026-07-05 | **P6-1/P6-2 checklist 同步**：核对 [P6 KAD 设计 spec](superpowers/specs/2026-07-04-ed2k-p6-kad-design.md) 与 [P6 KAD 实现计划](superpowers/plans/2026-07-04-ed2k-p6-kad-impl-plan.md) 已存在并覆盖 KadID/路由表/Kad2 UDP/搜索/发布/buddies/live 计划，勾选 RELEASE-PLAN 的 P6-1/P6-2；下一项为 P6-3 KadID + 路由表 + nodes.dat。文档同步，无代码改动，沿用上一轮 `ctest -C Debug -j8 --output-on-failure`：288 测试 281 pass + 7 live skip，0 fail | `f0871a7` |
...
| 2026-07-05 | **P7-1/P7-2 checklist 同步**：核对 [P7 协议混淆设计 spec](superpowers/specs/2026-07-04-ed2k-p7-obfuscation-design.md) 与 [P7 协议混淆实现计划](superpowers/plans/2026-07-04-ed2k-p7-obfuscation-impl-plan.md) 已存在并覆盖 MD5/RC4、TCP/UDP/服务器混淆、协商 fallback、live 计划、G1-G8 陷阱与 S1-S6 测试门；下一项为 P7-3 MD5/RC4 自带实现。文档同步，无代码改动，沿用上一轮 Windows/WSL 352 测试全绿证据 | 本地提交 |
...
| 2026-07-05 | **P8-1/P8-2 checklist 同步**：核对 [P8 客户端基础设施设计 spec](superpowers/specs/2026-07-04-ed2k-p8-client-infra-design.md) 与 [P8 客户端基础设施实现计划](superpowers/plans/2026-07-04-ed2k-p8-client-infra-impl-plan.md) 已存在并覆盖 IPFilter、Preferences、Statistics、Category、FriendList、ClientList、Proxy、HTTPThread、collection、preview、scheduler、ChatRelay、M1-M5、S1-S8、G1-G10 与 live 计划；下一项为 P8-3 IPFilter。文档同步，无代码改动，沿用上一轮 Windows/WSL 372 测试全绿证据；按用户要求仅本地提交，未 push/未触发 CI | 本地提交 |
...
| 2026-07-05 | **P9-1/P9-2 checklist 同步**：核对 [P9 eD2k 协议完备性设计 spec](superpowers/specs/2026-07-04-ed2k-p9-protocol-completeness-design.md) 与 [P9 eD2k 协议完备性实现计划](superpowers/plans/2026-07-04-ed2k-p9-protocol-completeness-impl-plan.md) 已存在并覆盖服务器 UDP 完备、服务器列表交换、MuleInfo、AICH 共享端、压缩块上传、aMule-faithful `.part.met`、大文件边界、S1-S7、G1-G9 与 live 计划；下一项为 P9-3 服务器 UDP 完备。文档同步，无代码改动，沿用 P8-7 Windows/WSL 420 测试全绿证据；按用户要求仅本地提交，未 push/未触发 CI | 本地提交 |
