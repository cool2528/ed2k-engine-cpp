...
>
> **最近更新**：2026-07-05 — P6-3 KadID + 路由表 + nodes.dat 完成（本地提交 `e329bfa`）：新增 128-bit `KadID`、k=10 LRU bucket、按本机 ID 范围分裂/合并的 `RoutingZone`/`RoutingTable`、最近 k 查询、compact `nodes.dat` round-trip；下一实施项为 P6-4 Kad2 UDP + Hello + bootstrap。
...
| **v1.2** | P6 KAD | Kad 路由表 + bootstrap + Kad2 UDP + 搜索/取源/发布 + buddies + notes（spec/plan 已起草） | 🚧 进行中：P6-3 ✅；P6-4 Kad2 UDP + Hello + bootstrap 下一项（[spec](superpowers/specs/2026-07-04-ed2k-p6-kad-design.md)/[plan](superpowers/plans/2026-07-04-ed2k-p6-kad-impl-plan.md)） |
