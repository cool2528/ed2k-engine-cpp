...
>
> **最近更新**：2026-07-05 — P6-7 集成 + CLI + live 完成（本地提交 `6deabe6`）：`MultiSourceDownload` 接入 Kad 找源，CLI 新增 `kad-bootstrap`/`kad-search`/`kad-find-sources`/`kad-publish`，并修正 Kad UInt128/tag/UDP crypto 搜索互操作；公网 `nodes.dat` live bootstrap + `kad-search ubuntu` 绿。下一实施项为 P7。
