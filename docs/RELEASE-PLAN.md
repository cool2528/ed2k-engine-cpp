...
| **P4c-3 多源并发 + 续传增强** | raccoon 多源并发 + 块粒度续传 + `peer::Block` u64 + 异步磁盘 I/O + .part.met 接入下载流程 | 🚧 M1✅ M2✅（raccoon 并发骨架+多源 e2e+.part.met 续传绿）/ M3/M4 待（spec✓ Block u64✓） |
...
> **P4c-3 M1+M2 验收 ✅**（2026-07-03）：raccoon 多源并发骨架（S1–S4）+ `.part.met` 续传接入全绿，222/227/5。**剩余 v1.0 硬范围**：M3 异步磁盘 I/O（P4c-3-7）、M4 live 验证 + >4GiB 边界（P4c-3-8 剩余）。
