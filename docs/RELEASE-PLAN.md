...
>
> **最近更新**：2026-07-04 — **🎉 v1.0.0 已发布**（tag `v1.0.0` @ `60f15de`，[GitHub Release](https://github.com/cool2528/ed2k-engine-cpp/releases/tag/v1.0.0)）。CI 双平台绿 + 全部发布检查清单 ✅。**P5-P9 全功能 spec/plan 已起草冻结**（对标 aMule 全部引擎功能除 UI）。
...
| **v1.1** | P5 上传/分享/信用 | 共享索引 + 服务器发布 + 上传会话 + 上传队列+限速 + 信用系统 + SourceExchange v2 + 评论/评分（spec/plan 已起草） | ⬜ 未开始（[spec](superpowers/specs/2026-07-04-ed2k-p5-upload-sharing-credits-design.md)/[plan](superpowers/plans/2026-07-04-ed2k-p5-upload-sharing-credits-impl-plan.md)） |
...
| **v2.1** | P8 客户端基础设施 | IPFilter + Preferences + Statistics + Category + FriendList + ClientList + Proxy + HTTPThread + .emulecollection + preview + scheduler（spec/plan 已起草） | ⬜ 未开始（[spec](superpowers/specs/2026-07-04-ed2k-p8-client-infra-design.md)/[plan](superpowers/plans/2026-07-04-ed2k-p8-client-infra-impl-plan.md)） |
...
| **P5 上传/分享/信用** | L4：共享索引(known.met/known2.met)、OP_OFFERFILES 发布、上传会话、上传队列+限速、ClientCredits、SourceExchange v2、评论/评分 | 有来有往的节点 | ⬜ 未开始 | spec/plan 已起草 |
...
| **P8 客户端基础设施** | L7：IPFilter、Preferences、Statistics、Category、FriendList、ClientList、Proxy、HTTPThread、.emulecollection、preview、scheduler | 完整客户端管理能力 | ⬜ 未开始 | spec/plan 已起草 |
...
- [ ] **P5-1 设计 spec**（✅ 已起草 2026-07-04）：L4 架构 + 逐帧字节级 + M1-M4 里程碑 + G1-G8 陷阱。
- [ ] **P5-2 实现计划**（✅ 已起草 2026-07-04）：S1-S8 green-deployable + 测试门 + 破改分析。
- [ ] **P5-3 共享索引 + known.met/known2.met**（M1/S1）：`KnownFileDB` + `known.met`/`known2.met` round-trip + 扫目录建索引 + PartFile→KnownFile 迁移。
- [ ] **P5-4 服务器发布 OP_OFFERFILES**（M2/S2）：`publish_files` + tag 顺序/类型（G4）+ CLI `publish`。
- [ ] **P5-5 上传会话**（M2/S3-S4）：`UploadSession` 响应 FILEREQ/FILESTATUS/HASHSET/REQUESTPARTS/AICH*（下载端协议的服务端镜像，对称复用）+ 读盘卸载。
- [ ] **P5-6 上传队列 + 限速 + 信用**（M3/S5-S7）：`UploadQueue` + slot 调度 + `UploadBandwidthThrottler` token bucket + `ClientCredits`(clients.met) + 信用分影响 slot。
- [ ] **P5-7 SourceExchange v2 + 评论/评分**（M4/S8）：`OP_ASKSHAREDFILES`/`SHAREDFILES`/`OP_SOURCEEXT`(SX2) + `OP_FILEDESC` + live 对 aMule peer。
- [ ] **P5-8 测试 + live**：mock 上传回环 + live（本地 aMule peer 互传源/上传）。
...
- [ ] **P8-6 Proxy + HTTPThread + collection + preview + scheduler**（M5/S6-S7）：SOCKS5/HTTP 代理 + HTTP 自动更新 + .emulecollection + OP_PREVIEWREQ/ANS + cron 调度器。
- [ ] **P8-7 集成 + CLI + live**（S8）：统一 CLI + 启动流程 + live IPFilter/更新/代理。
