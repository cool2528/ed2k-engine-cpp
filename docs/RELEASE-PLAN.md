...
>
> **最近更新**：2026-07-05 — P8-4 Preferences + Statistics 完成：新增 `infra::Preferences`（引擎配置内部格式 round-trip + `ed2k-tool config`）与 `infra::Statistics`（session/cumulative 计数 + 异步 flush + `ed2k-tool stats`）；下一实施项为 P8-5 Category + FriendList + ClientList。
...
| **v2.1** | P8 客户端基础设施 | IPFilter + Preferences + Statistics + Category + FriendList + ClientList + Proxy + HTTPThread + .emulecollection + preview + scheduler + **ChatRelay（OP_MESSAGE 中继）**（spec/plan 已起草） | 🚧 进行中：P8-1..P8-4 ✅；P8-5 Category + FriendList + ClientList 下一项（[spec](superpowers/specs/2026-07-04-ed2k-p8-client-infra-design.md)/[plan](superpowers/plans/2026-07-04-ed2k-p8-client-infra-impl-plan.md)） |
...
| **P8 客户端基础设施** | L7：IPFilter、Preferences、Statistics、Category、FriendList、ClientList、Proxy、HTTPThread、.emulecollection、preview、scheduler、ChatRelay | 完整客户端管理能力 | 🚧 进行中 | P8-1..P8-4 ✅；P8-5 下一项 |
