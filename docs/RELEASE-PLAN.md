...
>
> **最近更新**：2026-07-04（vcpkg baseline 修复 + CI workflow vcpkg versioning 修正 + 双平台构建验证进行中）
...
- [~] **R0-2 CI 流水线**（workflow vcpkg versioning 修正 2026-07-04，待 push 触发验证）
...
- [~] **R0-3 跨平台构建验证**（baseline 修复 2026-07-04；Windows + WSL Linux 构建进行中）
  ...
  - **本地 WSL Linux 验证 🚧（2026-07-04）**：vcpkg install 进行中（Boost 子包，~11min+）。configure + build + test 待。
  - 目标：双平台 `ed2k_tests` 全绿（229 pass + 5 live skip）。
