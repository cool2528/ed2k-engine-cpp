...
- [~] **R1-3（M1/P0）封装 + 线程安全类型强化**：`IPv4::value` 私有化 ✅ S1（`1b11513`，234/229/5 不回归）+ `MultiSourceDownload` Builder + 注入引用替换裸指针（S2 进行中）+ `set_disk_executor` 进构造 + disk pool 单线程断言 + `SharedState` executor 归属 debug 断言（S3 待）。
