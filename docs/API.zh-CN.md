...
// .part.met（aMule 0xE0/0xE2 格式；仍可解析旧版内部 gap 二进制数据）。E1：未完成但已写入部分
// 块的 part，除整 part 粒度的 gaps 外，还会额外携带一个私有扩展 tag（0xF1）记录其块级完成位图，
// 使续传能跳过已下载的块而非重下整个 part；不含该 tag 的旧版 .part.met 仍可正常加载（视为
// "无块级信息可恢复"，与 E1 之前的行为一致）。
...
class PartFile {
 public:
  ...
  void save_met() const;  // E1：显式落盘（暂停/取消/关闭路径在析构前调用）；
                          // 也会每 16 个完成块自动节流落盘一次，以及 MD4 校验失败重置时落盘
};
...
续传（已验证的分块不重新哈希）。E1：若某个 part 在 `pause()`/`cancel()`/`shutdown()`
打断时只下载了一部分，续传也会从其最后一次落盘的块级进度继续，而非从头重下——见上文
`PartFileState.partial_blocks` 说明。`cancel()` 在任意状态下都可调用，会将任务从注册表中
