...
**Hash bytes and parse a link** (no network, no Boost.Asio):

```cpp
#include <ed2k/hash.hpp>          // umbrella: core/hash + ed2k_hasher + aich_hasher
#include <ed2k/link.hpp>          // umbrella: link/ed2k_link
#include <iostream>

int main() {
  // Hash raw bytes (Red variant by default, matching aMule)
  const char data[] = "hello ed2k";
  auto hashed = ed2k::hash_bytes({reinterpret_cast<const std::byte*>(data), 10});
  std::cout << "MD4: " << hashed.file_hash.to_hex() << "\n";

  // Parse an ed2k:// link
  auto link = ed2k::parse_link(
      "ed2k://|file|example.bin|1024|31D6CFE0D16AE931B73C59D7E0C089C0|/");
  if (link) {
    auto& f = std::get<ed2k::Ed2kFileLink>(*link);
...
boost::asio::awaitable<void> run(boost::asio::any_io_executor ex) {
  ...
  server::LoginParams login;
  login.user_hash = *UserHash::from_hex("0123456789abcdef0123456789abcdef");
  login.client_port = 4662;
  login.nickname = "ed2k-consumer";

  auto id = co_await srv.connect_and_login(
      IPv4::from_dotted("1.2.3.4").value(), 4661, login,
      std::chrono::seconds(10));
  if (!id) co_return;

  auto results = co_await srv.search(server::Keyword{"ubuntu"},
...
int main() {
  ...
  rt.co_spawn_detached(run(rt.executor()));
  rt.run();
}
```
